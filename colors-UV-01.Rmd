...
if( !all(file.exists("./input_files/data_climate.tsv", "./input_files/data_humidity.tsv", "./input_files/data_dist2water.tsv", "./input_files/data_UV_incidence.tsv")) )
{
...
# Add the climate, humidity and distance2water variables,
# both for the languages and for the families.
...
# - altitude: log(+1) because there are two altitudes set to 0 meters, so increase everything by 1m to avoid -Inf:
d_colors$altitude_r           <- log2(d_colors$altitude + 1);
...
d_colors$longitude_family_180 <- ifelse(d_colors$longitude <= 180, d_colors$longitude, d_colors$longitude - 360);
...
We obtained altitude (in meters) data from @bentz_evolution_2018; please note that for the analyses, we use `log(altitude + 1)` (we added 1m to avoid errors for the locations recorded at sea level [0m]).

```{r summary altitiude, echo=FALSE, message=FALSE, warning=FALSE}
# Altitude (log)
pander(summary(d_colors$altitude));
```

```{r distribution of altitude, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of altitude.")}
ggplot(d_colors, aes(x=altitude)) +
  theme_bw() +
...
```{r map altitude, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of altitude (color scale).")}
ggplot() + theme_bw() +
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
  geom_label_repel(data=d_colors, aes(x=longitude, y=latitude, label=altitude), color="black", fill="lightyellow", alpha=0.75, size=2.75, label.padding=0.10, label.r=0.05) + 
  geom_point(data=d_colors, shape=21, aes(x=longitude, y=latitude, fill=altitude), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
