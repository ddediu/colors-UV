...
- use `macroarea2` (Eurasia -> Europe + Asia) instead of `macroarea`?
- add `climate_PC3` and `humidity`: **DONE**
- add distance2water
- add linguistic distances using ASJP and WALS
- add other structural features from WALS
...
- remove `gen.dist.na` and keep only the imputed genetic distances
...
if( !file.exists("./input_files/data_climate.tsv") || 
    !file.exists("./input_files/data_humidity.tsv") )
{
  # Get the primary data and compute them:
  source("./00_preprocess_environmental_data.R", echo=FALSE);
}
...
# Add the climate and humidity variables:
...
d_colors <- merge(d_colors, d_climate, by="glottocode", all.x=TRUE, all.y=FALSE);
d_colors <- merge(d_colors, d_humidity[,c("glottocode", 
                                          "humidity_mean_annual_median", "humidity_mean_annual_IQR")], # keep only the mean annual median and IQR as all the others are very highly correlated
                  by="glottocode", all.x=TRUE, all.y=FALSE);
names(d_colors)[ncol(d_colors)-c(1,0)] <- c("hum_median", "hum_IQR");

# Rescale variables (we use the suffix _r for them):
# - altitude:
d_colors$altitude_r         <- log2(d_colors$altitude + 1); # there are two altitudes set to 0 meters, so increase everything by 1m to avoid -Inf
# - distance to water bodies:
d_colors$dist2water_r       <- log2(d_colors$dist2water); # distances to water are never 0.0, so no need to guard against -Inf here
...
d_colors$latitude_r         <- cos(d_colors$latitude*(pi/180)); # cosine (degrees -> radians)
d_colors$longitude_r        <- cos(d_colors$longitude*(pi/180));
# - longitude and latitude of the language family origins:
d_colors$latitude_family_r  <- cos(d_colors$latitude_family*(pi/180)); # cosine (degrees -> radians)
d_colors$longitude_family_r <- cos(d_colors$longitude_family*(pi/180));
# - UVB:
d_colors$UVB_r              <- scale(d_colors$UVB); # z-score
# - daltonism:
...
In order to investigate our hypotheses, we analyzed a set of `r length(unique(d_colors$glottocode))` *populations* dispersed across all continents. 
This dataset was assembled from three sources: 

- first, the **data acompanying @brown_color_2004**, concerning 118 populations and including information on the frequency of color blindness, color vocabulary, and UVR incidence;
- second, **Emma Meeussen's [-@meeussen_colour_2015] expanded dataset**, who added many populations to the [@brown_color_2004] dataset, and changed some parameters;
...
Each population is identified by the [*glottocode*](https://glottolog.org/) [@hammarstrom_glottolog_2018] of the (primary) language it speaks, as well as by its *latitude*, *longitude* and *macroarea* (also from [Glottolog](https://glottolog.org/)).

#### Longitude

For the statistical analyses, longitude was transformed as described in [Data transformation].

```{r longitude summary, echo=FALSE, message=FALSE, warning=FALSE}
pander(summary(d_colors$longitude));
```

```{r longitude hist, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Histogram of longitude.")}
ggplot(d_colors, aes(x=longitude)) + 
  theme_bw() +
...
```{r longitude map, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of populations in our sample highlighting their longitude (color).")}
ggplot() + theme_bw() +
  theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, aes(x = longitude, y = latitude, color=longitude)) +
...
```{r latitude summary, echo=FALSE, message=FALSE, warning=FALSE}
pander(summary(d_colors$latitude));
```

```{r latitude hist, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Histogram of latitude.")}
ggplot(d_colors, aes(x=latitude)) + 
  theme_bw() +
  geom_histogram(color="tomato", fill="tomato", alpha=0.5)+
  #ggtitle("Distribution of latitude (rescaled)") +
  #theme(plot.title = element_text(size = 10, face = "bold")) + 
  NULL;
```

```{r latitude map, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of populations in our sample highlighting their latitude (color).")}
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, aes(x = longitude, y = latitude, color=latitude)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
```{r map macroareas, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of populations in our sample highlighting their macroarea (color).")}
# macroarea
pander(table(d_colors$macroarea));

ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=d_colors, aes(x = longitude, y = latitude, color=macroarea)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color = "Macroarea") +
...
In their previous investigations, @brown_color_2004  and @meeussen_colour_2015 gathered information about color vocabularies according to the definition of Basic Color Categories from @berlin_basic_1991. 
For this research, we selected only two variables from these linguistic data: the *size of the color lexicon*, and the *presence of a specific term for blue color*. 
The number of color categories (variable `n_colors`) varies between `r min(d_colors$n_colors, na.rm=TRUE)` and `r max(d_colors$n_colors, na.rm=TRUE)`, whereas variable `exists_blue` can only have the values `yes` or `no`.
...
ggplot(data =d_colors)+
  theme_bw() +
  geom_bar(aes(x=as.factor(n_colors), fill=as.factor(n_colors)), alpha=0.5) + 
  labs(x="Number of color categories", fill="")+
  guides(fill=FALSE)
```
...
ggplot() + theme_bw() + 
  ...
  labs(fill = "Nb color words") 
```


#### Specific term for exists_blue
...
```{r distribution word for blue, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Words for blue.")}
# DISTRIBUTION color vocab
ggplot(data =d_colors)+
  theme_bw() +
  geom_bar(aes(x=as.factor(exists_blue), fill=as.factor(exists_blue)), alpha=0.5) + 
  labs(x="Number of color categories", fill="")+
  guides(fill=FALSE)
```

```{r map word for blue, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of words for blue (color scale).")}
...
Data on *abnormal red-green* color perception *for males* was collected from 85 references, using the *Ishihara test* [@ishihara_tests_1917], the *anomaloscope* [@knoll_anomaloscope_1968], the *Holmgren-Thomson wool test* [[@thomson_instrument_1880]](https://www.psych.utoronto.ca/sites/www.psych.utoronto.ca/files/Professor%20Holmgren%E2%80%99s%20Test%20For%20Colour%20Blindness.pdf) or the *Hardy-Rand-Rittler pseudoisochromatic plate test* [[@hardy_hrr_1954]](https://www.good-lite.com/Details.cfm?ProdID=107). 

These data represents the percentage of red-green "color blind" males in the population, and varies between 0% and 11%. 
Overall "color blindness" rates were used, but when specific information was available, "color blindness" rate exclusively measures *deuteranopia*, *deuteranomaly*, *protanopia* and *protanomaly*. 
Thus, data on *tritanopia* was not included in this variable, since this implies abnormal color perception in the yellow-blue range. 
Samples where no subdivision between male and female was made, and containing data for less than 50 people, were excluded from this analysis. 
...
```{r distribution of daltonism, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Incidence of daltonism.")}
# Plot
ggplot(d_colors, aes(x=daltonism)) + 
  theme_bw() +
  geom_histogram(color="tomato", fill="tomato", alpha=0.5)+
  xlab("Incidence of daltonism (%)") +
  ggtitle("Distribution of daltonism") +
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map word for daltonism, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of incidence of daltonism (color scale).")}
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=daltonism), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "Freq daltonism (%)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
...
The *UVR incidence* was calculated from the data available from the NASA Total Ozone Mapping Spectrometer (TOMS) at http://toms.gsfc.nasa.gov/ery_uv/new_uv/ for the whole year 1998. 
This variable is a measure of UVR radiation received by the human body and measured in joules per square meter (J/m^2^), and takes into account the thickness of the ozone layer in the stratosphere, the amount of cloud cover, the elevation, and how high the sun is in the sky. 
Please note that, for statistical analyses, this variable was z-scored.

```{r summary UVB, echo=FALSE, message=FALSE, warning=FALSE}
# Visualize data
pander(summary(d_colors$UVB))
```

```{r distribution of UVB, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of UVR.")}
# Plot UVB
ggplot(d_colors, aes(x=UVB)) + 
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
  ggtitle("Distribution of UVR") +
  xlab("UVR incidence (J/m2)") + 
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map UVB, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of incidence of UVR (color scale).")}
# plot UVB on map
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=UVB), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "UVR incidence (J/m2)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
...
Building on @bentz_evolution_2018, historical data on global weather and climate were obtained from [WorldClim](https://www.worldclim.org/), and 19 variables were extracted for the period 1960-1990, including various measures, such as temperature, seasonality or precipitation [see @bentz_evolution_2018]. 
A Principal Component Analysis (PCA) found that the first two principal components (PCs) explain most of the data and have meaningful interpretations: the PC1 explains 48.6% of the variance and reflects low seasonality, wet and hot climate, whereas PC2 explains 24.1% of the variance and reflects high seasonality, hot and dry climate [see @bentz_evolution_2018 for details]. 
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=climate_PC1), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=climate_PC2), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
Altitude data was obtained from @bentz_evolution_2018; please note that for the analyses, we use `log`(altitude).

```{r summary altitiude, echo=FALSE, message=FALSE, warning=FALSE}
# Altitude (log)
pander(summary(log(d_colors$altitude)))
```

```{r distribution of altitude, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of log(altitude).")}
ggplot(d_colors, aes(x=log(altitude))) +
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
  ggtitle("Distribution of log(altitude)") +
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map altitude, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log(altitude) (color scale).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=log(altitude)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log altitude (m)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
```
...
Distances to lakes, oceans, rivers, and water in general, were also obtained from @bentz_evolution_2018; they were computed using OpenStreetMap raster file, while distance to water is the minimum distance to any sort of body of water. 
...
# Distance to lakes
pander(summary(log(d_colors$dist2lakes)));
```

```{r distribution dist lakes, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of log distance to lakes.")}
ggplot(d_colors, aes(x=log(dist2lakes))) +
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
  #ggtitle("Distribution of log distance to lakes") +
  xlab("log dist lakes (km)") + 
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map dist lakes, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log distance to lakes (color scale).")}
ggplot() + 
  theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=log(dist2lakes)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log dist lakes (km)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
```
...
# Distance to lakes
pander(summary(log(d_colors$dist2rivers)));
```

```{r distribution dist rivers, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of log distance to rivers.")}
ggplot(d_colors, aes(x=log(dist2rivers))) + 
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
  #ggtitle("Distribution of log distance to rivers") +
  xlab("log dist rivers (km)") + 
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map dist rivers, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log distance to rivers (color scale).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=log(dist2rivers)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log dist rivers (km)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
```
...
# Distance to oceans
pander(summary(log(d_colors$dist2ocean)));
```

```{r distribution dist oceans, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of log distance to oceans.")}
ggplot(d_colors, aes(x=log(dist2ocean))) + 
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
  #ggtitle("Distribution of log distance to oceans") +
  xlab("log dist oceans (km)") + 
  theme(plot.title = element_text(size = 10, face = "bold")); 
```

```{r map dist oceans, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log distance to oceans (color scale).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=log(dist2ocean)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log dist oceans (km)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
```
...
# Distance to water
pander(summary(log(d_colors$dist2water)));
```

```{r distribution dist water, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Distribution of log distance to water.")}
ggplot(d_colors, aes(x=log(dist2water))) + 
  theme_bw() +
  geom_density(color="tomato", fill="tomato", alpha=0.5)+
...
```{r map dist water, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log distance to water (color scale).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=log(dist2water)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log dist water (km)") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  NULL;
```
...
Subsistence strategy was obtained from [AUTOTYP](http://www.autotyp.uzh.ch/) [@bickel_autotyp_2017] as coded in [@blasi_human_2019], and represented by the binary variable `subsistence`: "yes" represents a population whose subsistence mode is based on hunting, fishing, gathering and/or foraging, whereas a "no" means that the subsistence mode of the population is centered around food production. 
As these data did not cover all the populations in our dataset, we expanded it by looking at other databases, such as [D-Place](https://d-place.org/) [@kirby_dplace_2016] and [Seshat](http://seshatdatabank.info/) [@turchin_seshat_2015]. 

```{r summary subsistence, echo=FALSE, message=FALSE, warning=FALSE}
# Hunter gatherer (yes/no)
...
ggplot(data =d_colors)+
  theme_bw() +
  geom_bar(aes(x=as.factor(subsistence), fill=as.factor(subsistence)), alpha=0.5) + 
  labs(x="Hunting-gathering subsistence strategy?", fill="")+
  guides(fill=FALSE)
```

```{r map subsistence, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of subsistence strategy (hunting-gathering).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
...
Population size was obtained from [@bentz_evolution_2018], and we `log`'ed it.
...
language family was obtained from [@bentz_evolution_2018]; there are `r length(unique(d_colors$glottocode_family))` uniques language families, most language sbelonging to the Indo-European, Afro-Asiatic and Uralic families. 
The putative geographic origins of language families (latitude and longitude) were obtained from [@wichmann_homelands_2010] supplemented with information from [Glottolog](https://glottolog.org/) [@hammarstrom_glottolog_2018].
...
# language family
pander(summary(d_colors$glottocode_family))
```

```{r distribution families, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Language families. Only the 15 most represented language families are explicitely shown, the other being gathered in the umbrella category 'Other'.")}
ggplot(data =d_colors)+
  theme_bw() +
...
```{r map family, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of main language families. Only the 15 most represented language families are explicitely shown, the other being gathered in the umbrella category 'Other'.")}
...
```{r map family origins, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Origins of the putative origins of language families.")}

# create sub dataframe without duplicates for supr_fam_name
subdf <- d_colors[!duplicated(d_colors$macro_family), ]


ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group), fill = "grey") +
  geom_point(data= d_colors, shape=21, aes(x = longitude_family, y = latitude_family), fill="red") +
  geom_text_repel(data= subdf, aes(x = longitude_family, y = latitude_family, label=macro_family),hjust=0, vjust=0, size=4) +

  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(x = "Longitude of language family", y= "Latitude of language family")
...
Unfortunately, there is apparently a lack of information concerning cross-population variation in the opsin genes (*OPN1MW* and *OPN1LW*). 
However, using a set of 96 micro-haplotypes and 382 SNP variants available from the [ALFRED database](https://alfred.med.yale.edu/) [@rajeevan_alfred_2003], we computed the overall genetic distance between population with [Arlequin](http://cmpg.unibe.ch/software/arlequin35/) [@excoffier_arlequin_2010]. 
Missing values in the final distance matrix were estimated using an ultrametric [@lapointe_estimating_1995] and additive [@de_soete_ultrametric_1984] data imputation methods (see [Appendix I. Distance matrices]).

Phylogenetic data was obtained from the [Glottolog](https://glottolog.org/) [@hammarstrom_glottolog_2018], and a distance matrix between our focus populations was created using [`qlcData`](https://github.com/cran/qlcData) and `cophenetic` in `R` [@R_2020].
For more details, please see the section about Point Pattern Analysis.
...
```{r summary genetic distance, include=FALSE, eval=FALSE}
# In this example, "add" means that this variable has been extracted from the "additive" distance matrix.
# V1 and V2 means that it is the first and second component from MultiDimensional Scaling. 
# For each distance matrix, MDS have been done with 2,3,5 component... The MDS extracted with 3 components are called "V1_add_3", "V2_add_3", "V3_add_3", etc.
summary(d_colors$V1_add_2)
summary(d_colors$V2_add_2)
```
...
Thus, a total of 19 *variables* (excluding the genetic multi-dimensional scaling dimensions) have been used for the analysis:
 ...
 -  *exists_blue*: is there of a specific term for describing the color blue in the language? (yes/no)
...
Five *distance matrices* were also collected among the populations (see [Appendix I. Distance matrices] and [Genetic distance from Alfred (FST)] for more info): 
 ...
 - one *genetic distance* matrix computed from ALFRED (see [Appendix I. Distance matrices]) without data imputation
 - one *genetic distance* matrix computed with ALFRED (see [Appendix I. Distance matrices]) with an *additive* data imputation
 - one *genetic distance* matrix computed with ALFRED (see [Appendix I. Distance matrices]) with an *ultrametric* data imputation
...
if( !file.exists("./cached_results/blue_uvb_best.RData") )
{
  ...
  # slightly worst with climate_PC2: 0.71 (Bayes factor)
  # slightly worst with climate_PC1: 0.66 (Bayes factor)
...
if( !file.exists("./cached_results/blue_var_importance.RData") )
{
  ...
  list_variables <- c("UVB_r" , "daltonism" , "subsistence" , "latitude_r", "longitude_r" , "log_popSize" , "climate_PC1", "climate_PC2" , "altitude_r"  , "dist2water_r", "dist2ocean_r" , "dist2lakes_r", "dist2rivers_r" , "n_colors" , "latitude_family_r", "longitude_family_r")
  ...
  for (it in 1:n_iter_gini)
  {
    ...
    rf.db = randomForest(exists_blue ~  UVB_r + daltonism  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = d_colors, importance=TRUE)
    ...
    obj <- cforest(exists_blue ~  UVB_r + daltonism  + subsistence +  latitude_r +longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = d_colors)
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all), 
       ...
       scale_x_discrete(labels=c("subsistence" = "Subsistence mode", "longitude_family_r" = "Long of family origin", "latitude_family_r" = "Lat of family origin", "dist2ocean_r" = "Dist to ocean","dist2rivers_r" = "Dist to rivers", "longitude_r" = "Longitude", "altitude_r" = "Altitude", "dist2water_r" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "dist2lakes_r" = "Dist to lakes", "latitude_r" = "Latitude", "log_popSize" = "Population size", "UVB_r" = "UVR","daltonism" = "daltonism", "n_colors" = "Nb of colour categ")) +
       guides(fill=FALSE)
```
...
if( !file.exists("./cached_results/blue_nonbcolors_var_importance.RData") )
{
  ...
  # choose variable to analyze
  list_variables <- c("UVB_r" , "daltonism"  , "subsistence" , "latitude_r", "longitude_r" , "log_popSize" , "climate_PC1", "climate_PC2" , "altitude_r"  , "dist2water_r", "dist2ocean_r" , "dist2lakes_r", "dist2rivers_r"  , "latitude_family_r", "longitude_family_r")
  ...
  for (it in 1:n_iter_gini){
    # compute random forest algorithm on all variable except multicollinear
    rf.db = randomForest(exists_blue ~  UVB_r + daltonism  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r + dist2water_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r  + latitude_family_r + longitude_family_r, data = d_colors, importance=TRUE)
    ...
    # use an other algorithm to compute the accuracy-based variable importance
    obj <- cforest(exists_blue ~  UVB_r + daltonism  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r  + latitude_family_r + longitude_family_r, data = d_colors)
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all), 
       ...
       theme(axis.text.x = element_text(angle = 60, hjust = 1, size =13, face = "bold"), 
             ...
             axis.title=element_text(size=16),
             legend.position="none") +
  scale_x_discrete(labels=c("subsistence" = "Subsistence mode", "longitude_family_r" = "Long of family origin", "latitude_family_r" = "Lat of family origin", "dist2ocean_r" = "Dist to ocean","dist2rivers_r" = "Dist to rivers", "longitude_r" = "Longitude", "altitude_r" = "Altitude", "dist2water_r" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "dist2lakes_r" = "Dist to lakes", "latitude_r" = "Latitude", "log_popSize" = "Population size", "UVB_r" = "UVR","daltonism" = "daltonism")) +
       guides(fill=FALSE)
```
...
-  daltonism and climate_PC1 are variables that have an impact on exists_blue in the random forest models, but not in the regression models,
...
# fit <- rpart(exists_blue ~ UVB + daltonism  + subsistence + longitude + latitude + log_popSize + climate_PC1 + climate_PC2 + altitude + dist2water + dist2ocean + dist2lakes + dist2rivers + n_colors, data = d_colors, method = 'class', model=TRUE, control=control)
#rpart.plot(fit, extra = 106)
tree <- ctree (exists_blue ~  UVB_r + daltonism  + subsistence + longitude_r + latitude_r + log_popSize + climate_PC1 + climate_PC2 + altitude_r + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, 
       data = d_colors, 
...
# create range
d_colors_map$PC2_range <- cut(d_colors_map$climate_PC2, c(-Inf, nb_PC2, Inf), 
...
if( !file.exists("./cached_results/blue_mach_learning.RData") )
{
  ...
  for (k in 1:30){
    ...
    rf.db = randomForest(exists_blue ~ UVB + daltonism  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = data_train)
    ...
    # fit model
    fit_train <- brm(exists_blue ~ UVB + daltonism  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r + (1 | macroarea) + (1 | glottocode_family), 
    ...
    tuned_parameters <- tune.svm(exists_blue ~ UVB + daltonism  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, 
                                 data = data_train, 
                                 gamma = 10^(-5:-1), 
                                 cost =10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(exists_blue ~ UVB + daltonism  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, 
                                data_train, 
                                kernel="radial", 
...
if( !file.exists("./cached_results/daltonism_var_importance.RData") )
{
  ...
  list_variables <- c("UVB_r" , "exists_blue"  , "subsistence" , "latitude_r", "longitude_r" , "log_popSize" , "climate_PC1", "climate_PC2" , "altitude_r"  , "dist2water_r", "dist2ocean_r" , "dist2lakes_r", "dist2rivers_r"  , "latitude_family_r", "longitude_family_r")
  ...
  for (it in 1:dn_iter_gini)
    ...
    # compute random forest algorithm on all variable except multicollinear
    rf.db = randomForest(daltonism ~  UVB_r + exists_blue  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r  + latitude_family_r + longitude_family_r, data = d_colors, importance=TRUE)
    ...
    # use an other algorithm to compute the accuracy-based variable importance
     obj <- cforest(daltonism ~  UVB_r + exists_blue  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2water_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r  + latitude_family_r + longitude_family_r, data = d_colors)
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all, fill=reorder(namevar, mean_all)), 
    ...
    scale_x_discrete(labels=c("subsistence" = "Subsistence mode", "longitude_family_r" = "Long of family origin", "latitude_family_r" = "Lat of family origin", "dist2ocean_r" = "Dist to ocean","dist2rivers_r" = "Dist to rivers", "longitude_r" = "Longitude", "altitude_r" = "Altitude", "dist2water_r" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "dist2lakes_r" = "Dist to lakes", "latitude_r" = "Latitude", "log_popSize" = "Population size", "UVB_r" = "UVR","exists_blue" = "exists_blue", "n_colors"="Nb of colour categ")) +
      guides(fill=FALSE)
```
...
tree2 <- ctree (daltonism ~  UVB_r + exists_blue  + subsistence + latitude_r + longitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, 
                data = d_colors, 
                control = ctree_control(testtype = "MonteCarlo", minsplit = 6, minbucket = 6, maxdepth=30))
...
if( !file.exists("./cached_results/daltonism_mach_learning.RData") )
{
  ...
  for (k in 1:30){
    ...
    # fit randomforest on training dat
    rf.db = randomForest(daltonism_r ~ UVB + exists_blue  + subsistence + longitude_r + latitude_r + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = data_train)
    ...
    brms_model <- brm(daltonism_r ~ UVB + exists_blue  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r + (1 | macroarea) + (1 | glottocode_family), 
                      family="beta",
                     data=data_train,
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
                     control=brms_control); 
    ...
    # find best parameters for SVM
    tuned_parameters <- tune.svm(daltonism_r ~ UVB + exists_blue  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = data_train, gamma = 10^(-5:-1), cost = 10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(daltonism_r ~ UVB + exists_blue  + subsistence + longitude_r + latitude_r  + log_popSize + climate_PC1 + climate_PC2 + altitude_r  + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data_train, kernel="radial", cost=tuned_parameters$best.parameters[1,2], gamma=tuned_parameters$best.parameters[1,1])
