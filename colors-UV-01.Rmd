...
- add `climate_PC3` and `humidity`
- keep only the libraries we actually use: **DONE**
- integrate FST_methods.Rmd in the "methods" section here
...
# - altitude:
d_colors$altitude_r         <- log2(d_colors$altitude + 1); # there are two altitudes set to 0 meters, so increse everything by 1m to avoid -Inf
...
Missing values in the final distance matrix were estimated using an ultrametric [@lapointe_estimating_1995] and additive [@de_soete_ultrametric_1984] data imputation methods (see [Appendix]).
...
Genetic distances were estimated using FST on data from the ALFRED database (see method in [Appendix]).
...
```{r Distance matrices, child='FST_method.Rmd', eval=TRUE}
