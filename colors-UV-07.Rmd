...
if( !all(file.exists(file_name)) )
{
  ...
  (hdi_full <- hdi(brms_blue__ncol, ci=0.95)); (rope_full <- rope(brms_blue__ncol));
  ...
  (hdi_reduced <- hdi(brms_blue__ncol_1, ci=0.95)); (rope_reduced <- rope(brms_blue__ncol_1));
  ...
  brms_blue$with_ncolors <- list("full"   =list("cmp_2_null"=cmp_null_full, 
                                 ...
                                 "reduced"=list("cmp_2_null"=cmp_null_reduced, "cmp_2_full"=cmp_full_reduced, 
                                                "HDI"=hdi_reduced, "ROPE"=rope_reduced, "confusionMatrix"=cm_reduced));
  ...
  (hdi_full <- hdi(brms_blue__noncol, ci=0.95)); (rope_full <- rope(brms_blue__noncol));
  ...
  (hdi_reduced <- hdi(brms_blue__noncol_1, ci=0.95)); (rope_reduced <- rope(brms_blue__noncol_1));
...
{
  # The data:
  d <- d_colors; d$macroarea <- as.factor(d$macroarea);
  ...
  ctree_blue <- list("data"=d,
                     "with_ncolors"=ctree_blue__ncol, "without_ncolors"=ctree_blue__noncol);
...
The two achieve comparable levels of data fit (top = with *n_colors*, bottom = without *n_colors*):

```{r}
confusionMatrix(ctree_blue$data$exists_blue, predict(ctree_blue$with_ncolors), positive="yes");
confusionMatrix(ctree_blue$data$exists_blue, predict(ctree_blue$without_ncolors), positive="yes");
```
...
if( !all(file.exists(file_name)) )
{
  ...
  n_replications <- 100; # the number of replications (there is some randomness involved)
...
if( !all(file.exists(file_name)) )
{
  ...
  # Collect the success measures and the predictor importance:
  n_replications <- 500; # the number of replications (there is some randomness involved)
  results <- pblapply(1:n_replications, function(i)
    {
      # split the dataset into a training (80%) and a testing (20%) set stratified by exists_blue:
      data_split <- rsample::initial_split(d, prop=0.80, strata="exists_blue");
      ...
      # with n_colors:
      success_ncol <- data.frame("replication"    =i, # replication
                                 "svm_accuracy"   =cm_svm_ncols$overall["Accuracy"],
                                 "svm_sensitivity"=cm_svm_ncols$byClass["Sensitivity"], 
                                 "svm_specificity"=cm_svm_ncols$byClass["Specificity"],
                                 "svm_precision"  =cm_svm_ncols$byClass["Precision"],
                                 "svm_recall"     =cm_svm_ncols$byClass["Recall"],
                                 row.names=NULL);
      pi_df_ncol   <- data.frame("replication"=i,                                   # replication
                                 "predictor"  =list_predictors_ncol,                # predictors
      ...
      # without n_colors:
      success_noncol <- data.frame("replication"    =i, # replication
                                   "svm_accuracy"   =cm_svm_noncols$overall["Accuracy"],
                                   "svm_sensitivity"=cm_svm_noncols$byClass["Sensitivity"], 
                                   "svm_specificity"=cm_svm_noncols$byClass["Specificity"],
                                   "svm_precision"  =cm_svm_noncols$byClass["Precision"],
                                   "svm_recall"     =cm_svm_noncols$byClass["Recall"],
                                   row.names=NULL);
      pi_df_noncol   <- data.frame("replication"=i,                                       # replication
                                   "predictor"  =list_predictors_noncol,                  # predictors
...
The SVMs are quite successful at predicting *blue*:

```{r}
tmp_ncol   <- vapply(svm_blue$with_ncolors$success[,-1],    function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp_noncol <- vapply(svm_blue$without_ncolors$success[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
...
kable(tmp, row.names=FALSE, col.names=c("With *n_colors*?", "Accuracy", "Sensitivity", "Specificity", "Recall"),
      caption=capTab(paste0("Various measures of success at predicting *blue* using SVMs with ",svm_blue$replications," training/testing sets, and including or not *n_colors*.")));
```

```{r fig.cap=capFig(paste0("Various measures of success using SVMs with ",svm_blue$replications," training/testing sets, including or not *n_colors*.")), fig.width=2*4, fig.height=5}
ggplot(rbind(cbind(svm_blue$with_ncolors$success, "with_ncol"="yes"),
             cbind(svm_blue$without_ncolors$success, "with_ncol"="no")) %>% 
         reshape2::melt(id.vars=c("replication", "with_ncol"), variable.name="measure", value.name="value") %>% 
         mutate("measure"=factor(substring(measure,5), levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), 
                "value"=value*100), 
       aes(x=measure, y=value, fill=with_ncol)) + 
  ylim(c(0,100)) + xlab("Measures of success") + ylab("Value (%)") +
  theme_bw() +
...
```{r fig.cap=capFig(paste0("Specificity-based predictor importance from SVMs using ",svm_blue$replications," traning/testing sets, and including or not *n_colors*.")), fig.width=4*3, fig.height=7*1}
grid.arrange(ggplot(rbind(cbind(svm_blue$with_ncolors$pred_importance, "with_ncol"="yes"), 
                          cbind(svm_blue$without_ncolors$pred_importance, "with_ncol"="no")), 
               ...
               coord_flip() +
               NULL,
             ggplot(svm_blue$with_ncolors$pred_importance, 
               ...
               coord_flip() + 
               NULL,
             ggplot(svm_blue$without_ncolors$pred_importance, 
                    aes(x=reorder(predictor, importance, median), y=importance)) +
               theme_bw() +
               geom_boxplot(fill="salmon", color="black") +
               labs(x="Predictors (sorted by decreasing importance)", y="Importance") +
