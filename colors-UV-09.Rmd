...
- keep only the needed input files: **DONE**
- do SEM/multiple mediators stuff as I did for syllable and tone -> modification indices: **DONE**
- redo the mediation analysis with the new flow blue -> n_colors: **DONE**
- change standardization of latitude so that it is highest at the poles: **DONE**
- redo the mediation using the two new functions: **DONE**
- update the summary mediation diagram: **DONE**
- prediction: put brms, trees, forests, svms together + on the data & training/testing sets: **DONE**
- discuss causality in these models: **DONE**
- check if we can get more lexical splits (such as grue -> green + blue): SEM/IRT?: **FOR THE FUTURE**
...
As per @brown_color_2004, we computed the cosine of these *latitude*s and the *longitude*s to be used in the statistical models; while `cos(latitude)` captures the closeness to the equator and ranging between 0.0 (one of the poles) and 1.0 (the equator), we use here `1.0 - cos(latitude)` so that they are 0.0 at the equator and 1.0 at the poles, converving the "natural" interpretation of latitudes, and `cos(longitude)`range between -1.0 and 1.0 (corresponding to -180 and 180 degrees, respectively); 
please note that longitude can be coded (and plotted) either as ranging between 0° and 360°, or between -180° and +180°, or, when we need to avoid the International Date Line (IDL) producing an artefactual boundary.
...
For the statistical analyses, *n_colors* was nomralized between 0 and 1 (thus, the relationship between raw and transformed values is linear).
...
```{r fig.cap=capFig("Visual representation of a mediation, showing the individual estimates (edges, $a$, $b$ and $c$) and the effects (total, direct and indirect). We may use line style and color to differentiate ns, negative and positive edges. Likewise, we may report the total, direct and indirect effects on the diagrem itself using colors to distinguish significant and ns effects.")}
# Mediation:
DiagrammeR::grViz('
...
Please note that it is unclear how we should model, in causal terms, the relationship between the number of color terms in a language, *n_colors*, and the existence of a dedicated word for "blue", *blue* (see the figure below): 

a) on the one hand, it could be argued that when the number of colors in a language reaches a certain threshold (around 6) this causes the split of the "grue" category in two, one for "green" and one for "blue",
b) on the other, it may be that the non-emergence of a dedicated word for "blue" is "blocking" the development of the color vocabulary and, therefore, the increase in the number of colors (beyond 6),
c) finally, we can imagine that both *n_colors* and *blue* are imperfect measures ("indicators") of a "deeper" latent (and not directly measurable) construct of "color vocabulary complexity", and that, therefore, there is a relationship between them due to both "loading" on this latent factor.

```{r fig.cap=capFig('Three possible views on the causal relationships between the number of color terms in a language, *n_colors*, and the existence of a dedicated word for "blue", *blue*. Left: *n_colors* &rarr; *blue*; center:  *blue* &rarr; *n_colors*; right: *n_colors* &larr; *CCL* &rarr; *blue*, where *CCL* is a latent (unmeasured) variables that captures something akin to the complexity of the color lexicon of a language.')}
DiagrammeR::grViz(paste0('
  digraph ncolors_blue_relationships {
  ...
  blue3  [label = "blue", tootlip="blue"]; 
  ncol3  [label = "n_colors", tootlip="n_colors"]; 
  latent [label = "CCL", tootlip="CCL", shape="ellipse"]; 
  label3 [label = "(C)", tooltip="(C)", shape="none", style="none"];

  blue2  [label = "blue", tootlip="blue"]; 
  ncol2  [label = "n_colors", tootlip="n_colors"]; 
  label2 [label = "(B)", tooltip="(B)", shape="none", style="none"];
  
  blue1  [label = "blue", tootlip="blue"]; 
  ncol1  [label = "n_colors", tootlip="n_colors"]; 
  label1 [label = "(A)", tooltip="(A)", shape="none", style="none"];

  # the edges:
  edge   [style = "solid", color = "black"];
  
  label1 -> ncol1 [style = "none", color = "none"];
  ncol1 -> blue1;
  
  label2 -> ncol2 [style = "none", color = "none"];
  ncol2 -> blue2 [dir="back"];
  
  label3 -> ncol3 [style = "none", color = "none"];
  ncol3 -> latent [dir="back"];
  latent -> blue3;
}
'));
```

However, (c) is not feasible with our data, which leaves the possibilities (a) and (b), my personal preference being for (b).
...
# Null model (intercept) -> n_colors:
if( !all(file.exists("./cached_results/b_0__ncol.RData")) )
{
  # check different distributions:
  b_0__ncol_gaussian <- brm(n_colors ~ 1 + (1 | glottocode_family) + (1 | macroarea), # null model for n_colors
  ...
  b_0__ncol_gaussian <- brms_fit_indices(b_0__ncol_gaussian);
  summary(b_0__ncol_gaussian); mcmc_plot(b_0__ncol_gaussian, type="trace"); mcmc_plot(b_0__ncol_gaussian);
  
  b_0__ncol_poisson <- brm(n_colors ~ 1 + (1 | glottocode_family) + (1 | macroarea), # null model for n_colors
                           family=poisson(), data=d_colors, 
                           prior=c(prior(student_t(3, 0, 2.5), class="Intercept")), # pretty wide priors centered on 0
                           save_all_pars=TRUE, # needed for Bayes factors
                           sample_prior=TRUE,  # needed for hypotheses tests
                           cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_0__ncol_poisson <- brms_fit_indices(b_0__ncol_poisson);
  summary(b_0__ncol_poisson); mcmc_plot(b_0__ncol_poisson, type="trace"); mcmc_plot(b_0__ncol_poisson);
  
  compare_0_ncol__gaussian_poisson <- brms_compare_models(b_0__ncol_gaussian, b_0__ncol_poisson, "gaussian", "poisson"); # gaussian fits better
  # so, even if nominally n_colors is a count, it seems to behave more like a continuous measure:
  b_0__ncol <- b_0__ncol_gaussian;
  
  # Save results:
  save(b_0__ncol, 
       compare_0_ncol__gaussian_poisson,
       file="./cached_results/b_0__ncol.RData", compress="xz", compression_level=9);
} else
{
  load("./cached_results/b_0__ncol.RData");
}
```
...
#### # of color terms & "blue" [@berlin_basic_1991's stages]
...
if( !all(file.exists("./cached_results/b_ncol__blue.RData")) )
{
  # n_colors:
  b_ncol__blue <- brm(exists_blue ~ 1 + n_colors_r + (1 | glottocode_family) + (1 | macroarea), # n_colors -> exists_blue
                      family=bernoulli(link="logit"), data=d_colors, 
  ...
  (h_ncol__blue <- brms::hypothesis(b_ncol__blue, c("n_colors_r = 0", "n_colors_r > 0")));
...
# Word for blue -> number of color categories:
if( !all(file.exists("./cached_results/b_blue__ncol.RData")) )
{
  # n_colors:
  b_blue__ncol <- brm(n_colors ~ 1 + exists_blue + (1 | glottocode_family) + (1 | macroarea), # exists_blue -> n_colors
                      family=gaussian(), data=d_colors, # even if nominally a count, this seems to behave more like a continuous measure (I also did model comparison with poisson)
                      prior=c(prior(normal(0, 5), class="Intercept"),
                              prior(normal(0, 5), class="b")), # pretty wide priors centered on 0
                      save_all_pars=TRUE, # needed for Bayes factors
                      sample_prior=TRUE,  # needed for hypotheses tests
                      cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_blue__ncol <- brms_fit_indices(b_blue__ncol);
  summary(b_blue__ncol); mcmc_plot(b_blue__ncol, type="trace"); mcmc_plot(b_blue__ncol);
  (h_blue__ncol <- brms::hypothesis(b_blue__ncol, c("exists_blueyes = 0", "exists_blueyes > 0")));
  (hdi_blue__ncol <- hdi(b_blue__ncol, ci=0.95));
  compare_0_blue__ncol <- brms_compare_models(b_0__ncol, b_blue__ncol, "null", "+ blue"); # clear positive effect of exists_blue == "yes"!

  # Save results:
  save(compare_0_blue__ncol, h_blue__ncol, hdi_blue__ncol, #b_blue__ncol, # no need to save the actual model
       file="./cached_results/b_blue__ncol.RData", compress="xz", compression_level=9);
} else
{
  load("./cached_results/b_blue__ncol.RData");
}
```
...
This is confirmed by a Bayesian mixed-effects regression of *n_colors* on the presence of a specific word for blue (*exists_blue*): *&beta;~exists_blue:"yes" vs "no"~* = `r round(h_blue__ncol$hypothesis$Estimate[1],2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_blue__ncol$CI_low[ hdi_blue__ncol$Parameter == "b_exists_blueyes" ], hdi_blue__ncol$CI_high[ hdi_blue__ncol$Parameter == "b_exists_blueyes" ])`), posterior probability that "yes"=="no", *p*(*&beta;*=0) = `r sprintf("%.4g",h_blue__ncol$hypothesis$Post.Prob[1])`; this effect should be specifically *positive*, which is clearly supported by the posterior probability that "yes">"no", *p*(*&beta;*>0) = `r sprintf("%.4g",h_blue__ncol$hypothesis$Post.Prob[2])`. It is interesting to note that, on average, the languages without a specific word for "blue" have about 5 color words (95%CI between about 4.4 and 6.0), while those with such a word add about 3.5 more (95%CI between about 2.8 and 4.2), having about 8.5 color words.

Conversely, the number of color categories (*n_colors*) predicts the presence of a specific word for blue (*exists_blue*), as shown by the plot below, and by a Bayesian mixed-effects logistic regression of *exists_blue* on *n_colors* (with family and macroarea as random effects): *&beta;~n_colors~* = `r round(h_ncol__blue$hypothesis$Estimate[1],2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_ncol__blue$CI_low[ hdi_ncol__blue$Parameter == "b_n_colors_r" ], hdi_ncol__blue$CI_high[ hdi_ncol__blue$Parameter == "b_n_colors_r" ])`), posterior probability that *&beta;~n_colors~*=0, *p*(*&beta;*=0) = `r sprintf("%.4g",h_ncol__blue$hypothesis$Post.Prob[1])`; this effect should be specifically *positive*, which is clearly supported by the posterior probability that *&beta;~n_colors~*>0, *p*(*&beta;*>0) = `r sprintf("%.4g",h_ncol__blue$hypothesis$Post.Prob[2])`.

```{r exploratory blue, fig.cap=capFig('The distribution of the number of color categories across languages with and without a specific word for "blue", showing the jittered data points, the densities (colored violins) and boxplots (black). The dashed dark gray horizontal line shows the 6 colors categories.'), fig.height=1*5, fig.width=1*4}
ggplot(d_colors, aes(y=n_colors, x=exists_blue, fill=exists_blue)) + 
  ...
  theme_bw() +
  labs(x="Word for blue?", y="Number of color categories") +
  ...
  geom_hline(yintercept = 6, color = "gray30", linetype="dashed") +
...
if( !all(file.exists("./cached_results/b_gen__blue.RData")) )
{
  ...
  compare_0_gen__blue <- brms_compare_models(b_0__blue, b_gen__blue_all, "null", "+ water"); # not really better...
  # Iterative manual simplification:
...
#### Subsistence &rarr; color vocabulary &rarr; "blue"

```{r include=FALSE}
file_name <- "./cached_results/bmed_subs_ncol__blue.RData";
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_subs_ncol__blue <- .fit_mediation_model(d=d_colors, 
                                               outcome="exists_blue", outcome_name="blue", 
                                               treatment="subsistence", treatment_name="subsistence", 
                                               mediator="n_colors_r", mediator_name="n_colors", 
                                               family_mediator=gaussian(), family_outcome=bernoulli("logit"), ranefs="(1 | glottocode_family) + (1 | macroarea)",
                                               cores=brms_ncores, iter=6000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10),
                                               save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_subs_ncol__blue, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_subs_ncol__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                 outcome=m$outcome, outcome_name=m$outcome_name, 
                                                 treatment=m$treatment, treatment_name=m$treatment_name, 
                                                 mediator=m$mediator, mediator_name=m$mediator_name);
```
...
#### Subsistence &rarr; "blue" &rarr; color vocabulary

```{r include=FALSE}
file_name <- "./cached_results/bmed_subs_blue__ncol.RData";
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_subs_blue__ncol <- .fit_mediation_model(d=d_colors, 
                                               outcome="n_colors", outcome_name="n_colors", 
                                               treatment="subsistence", treatment_name="subsistence", 
                                               ...
                                               save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_subs_blue__ncol, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
m <- bmed_subs_blue__ncol; .plot_mediation_model(m$summary, m$mediation, 
                                                 outcome=m$outcome, outcome_name=m$outcome_name, 
                                                 treatment=m$treatment, treatment_name=m$treatment_name, 
                                                 mediator=m$mediator, mediator_name=m$mediator_name);
```
...
#### Population size &rarr; color vocabulary &rarr; "blue"

```{r include=FALSE}
file_name <- "./cached_results/bmed_pops_ncol__blue.RData"; 
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_pops_ncol__blue <- .fit_mediation_model(d=d_colors, 
                                               outcome="exists_blue", outcome_name="blue", 
                                               treatment="log_popSize", treatment_name="pop_size", 
                                               mediator="n_colors_r", mediator_name="n_colors", 
                                               family_mediator=gaussian(), family_outcome=bernoulli("logit"), ranefs="(1 | glottocode_family) + (1 | macroarea)",
                                               cores=brms_ncores, iter=6000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10),
                                               save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_pops_ncol__blue, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_pops_ncol__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                 outcome=m$outcome, outcome_name=m$outcome_name, 
                                                 treatment=m$treatment, treatment_name=m$treatment_name, 
                                                 mediator=m$mediator, mediator_name=m$mediator_name); 
```
...
#### Population size &rarr; "blue" &rarr; color vocabulary

```{r include=FALSE}
file_name <- "./cached_results/bmed_pops_blue_ncol.RData"; 
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_pops_blue_ncol <- .fit_mediation_model(d=d_colors, 
                                               outcome="n_colors", outcome_name="n_colors", 
                                               treatment="log_popSize", treatment_name="pop_size", 
                                               mediator="exists_blue", mediator_name="blue", 
                                               family_mediator=bernoulli("logit"), family_outcome=gaussian(), ranefs="(1 | glottocode_family) + (1 | macroarea)",
                                               ...
                                               save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_pops_blue_ncol, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the mediator is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_pops_blue_ncol; .plot_mediation_model(m$summary, m$mediation, 
                                                 outcome=m$outcome, outcome_name=m$outcome_name, 
                                                 treatment=m$treatment, treatment_name=m$treatment_name, 
                                                 mediator=m$mediator, mediator_name=m$mediator_name); 
```
...
#### UV-B &rarr; color vocabulary &rarr; "blue"

```{r include=FALSE}
file_name <- "./cached_results/bmed_uvb_ncol__blue.RData";
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_uvb_ncol__blue <- .fit_mediation_model(d=d_colors, 
                                              outcome="exists_blue", outcome_name="blue", 
                                              ...
                                              save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_uvb_ncol__blue, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_uvb_ncol__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name); 
```
...
#### UV-B &rarr; "blue" &rarr; color vocabulary

```{r include=FALSE}
file_name <- "./cached_results/bmed_uvb_blue__ncol.RData";
if( !all(file.exists(file_name)) )
{
  # Fit the mediation model:
  bmed_uvb_blue__ncol <- .fit_mediation_model(d=d_colors, 
                                              outcome="n_colors", outcome_name="n_colors", 
                                              treatment="UVB_r", treatment_name="UV-B", 
                                              mediator="exists_blue", mediator_name="blue", 
                                              family_mediator=bernoulli("logit"), family_outcome=gaussian(), ranefs="(1 | glottocode_family) + (1 | macroarea)",
                                              ...
                                              save_model=FALSE, show_results=TRUE);
  # Save results:
  save(bmed_uvb_blue__ncol, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name);
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the mediator is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_uvb_blue__ncol; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name); 
```
...
#### Extra: what's going on with dist2lakes and climate?

Is the distance to lakes a proxy for something else, such as climate, geographic location or UV-B incidence?
...
if( !all(file.exists(file_name)) )
{
  ...
  # dist2lakes -> latitude -> "blue":
  bmed_d2l_lat__blue <- .fit_mediation_model(d=d_colors, 
                                             outcome="exists_blue", outcome_name="blue", 
  ...
  bmed_d2l_clim__blue <- .fit_mediation_model(d=d_colors, 
                                              outcome="exists_blue", outcome_name="blue", 
                                              treatment="dist2lakes_r", treatment_name="dist2lakes", 
                                              mediator="clim_PC1_r", mediator_name="climate PC1", 
                                              family_mediator=gaussian(), family_outcome=bernoulli("logit"), ranefs="(1 | glottocode_family) + (1 | macroarea)",
  ...
  bmed_d2l_uvb__blue <- .fit_mediation_model(d=d_colors, 
                                             outcome="exists_blue", outcome_name="blue", 
                                             treatment="dist2lakes_r", treatment_name="dist2lakes", 
  ...
  # dist2lakes -> n_colors -> "blue":
  bmed_d2l_ncol__blue <- .fit_mediation_model(d=d_colors, 
                                              outcome="exists_blue", outcome_name="blue", 
                                              treatment="dist2lakes_r", treatment_name="dist2lakes", 
                                              mediator="n_colors_r", mediator_name="n_colors", 
                                              family_mediator=gaussian(), family_outcome=bernoulli("logit"), ranefs="(1 | glottocode_family) + (1 | macroarea)",
  ...
  # dist2lakes -> "blue" -> n_colors:
  bmed_d2l_blue__ncol <- .fit_mediation_model(d=d_colors, 
                                              outcome="n_colors", outcome_name="n_colors", 
                                              treatment="dist2lakes_r", treatment_name="dist2lakes", 
                                              mediator="exists_blue", mediator_name="blue", 
                                              family_mediator=bernoulli("logit"), family_outcome=gaussian(), ranefs="(1 | glottocode_family) + (1 | macroarea)",
  ...
  # Save results:
  save(bmed_d2l_lat__blue, bmed_d2l_clim__blue, bmed_d2l_uvb__blue, bmed_d2l_ncol__blue, bmed_d2l_blue__ncol, file=file_name, compress="xz", compression_level=9);
} else
{
  load(file_name); 
}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_d2l_lat__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name);
```
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_d2l_clim__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name);
```
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_d2l_uvb__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name);
```
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_d2l_ncol__blue; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name);
```
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the mediator is binary, the direct and indirect effects may be on different scales.'), fig.width=10, fig.height=5}
m <- bmed_d2l_blue__ncol; .plot_mediation_model(m$summary, m$mediation, 
                                                outcome=m$outcome, outcome_name=m$outcome_name, 
                                                treatment=m$treatment, treatment_name=m$treatment_name, 
                                                mediator=m$mediator, mediator_name=m$mediator_name);
```
...
Thus, for **view (a) [*n_colors* &rarr; *blue*]**, the data supports the following pathways:

```{r fig.cap=capFig('The links supported by the mediation and regression analyses. Edges: solid blue = negative effects, solid red = positive effects, and dashed gray = null effects; the dashed red edge is borderline significant. Nodes: "blue" in blue is the outcome, "n_colors" and "UV-B" in green are of particular interest here, light yellow have an effect on "blue", while light gray do not. This should *not* be interpreted as a path diagram!'), fig.width=7, fig.height=4}
DiagrammeR::grViz(paste0('
  digraph full_blue_a {
  ...
  ncol [label = "n_colors", tooltip = "n_colors", fillcolor="seagreen1"]; 
  ...
  subs [label = "subsistence", tooltip = "subsistence", fillcolor="gray90", fontcolor="gray50"]; 
  psiz [label = "pop_size", tooltip = "pop_size"]; 
  d2l  [label = "dist2lakes", tooltip = "dist2lakes"]; 

  # the edges:
  edge [style = "solid", color = "black"];
  ncol -> blue [label="+", color="red", fontcolor="red"];
  UVB  -> blue [label="-", color="blue", fontcolor="blue"];
  lat  -> blue [color="gray80", fontcolor="gray80", style="dashed", label=""];
  lat  -> UVB  [label="-", color="blue", fontcolor="blue"];
  lat  -> clim [label="+", color="red", fontcolor="red"];
  clim -> blue [label="(+)", color="red", fontcolor="red", style="dashed"];
  subs -> blue [color="gray80", fontcolor="gray80", style="dashed", label=""];
  subs -> ncol [color="gray80", fontcolor="gray80", style="dashed", label=""]; 
  psiz -> ncol [label="+", color="red", fontcolor="red"];
  psiz -> blue [color="gray80", fontcolor="gray80", style="dashed", label=""];
  UVB  -> ncol [label="-", color="blue", fontcolor="blue"];
  d2l  -> blue [label="-", color="blue", fontcolor="blue"];
  d2l  -> lat  [label="-", color="blue", fontcolor="blue"];
  d2l  -> clim [color="gray80", fontcolor="gray80", style="dashed", label=""];
  d2l  -> UVB  [label="+", color="red", fontcolor="red"];
  d2l  -> ncol [label="-", color="blue", fontcolor="blue"];
}
'))
```

- *latitude* has no direct effect on "blue", but is mediated mostly by the negative one of *UV-B incidence* and, possibly, by the positive one of *climate PC1*;
- *number of color categories* has a positive effect on "blue";
- *subsistence strategy* does not affect "blue";
- *population size* affects "blue" only through the *number of color categories*;
- *distance to lakes* has a direct negative effect on "blue", but also indirect effects through *UV-B incidence*, *latitude* and *number of color categories*.


For **view (b) [*blue* &rarr; *ncolors*]**, the data supports the following pathways:

```{r fig.cap=capFig('The links supported by the mediation and regression analyses. Conventions as above.'), fig.width=7, fig.height=4}
DiagrammeR::grViz(paste0('
  digraph full_blue_b {
  ...
  # the edges:
  edge [style = "solid", color = "black"];
  blue -> ncol [label="+", color="red", fontcolor="red"];
  UVB  -> blue [label="-", color="blue", fontcolor="blue"];
  lat  -> blue [color="gray80", fontcolor="gray80", style="dashed", label=""];
  lat  -> UVB  [label="-", color="blue", fontcolor="blue"];
  lat  -> clim [label="+", color="red", fontcolor="red"];
...
- *latitude* has no direct effect on "blue", but is mediated mostly by the negative one of *UV-B incidence* and, possibly, by the positive one of *climate PC1*;
- *number of color categories* is positively affected by "blue";
- *subsistence strategy* does not affect "blue";
- *population size* has a positive effect on "blue";
- *distance to lakes* has a direct negative effect on "blue", but also indirect effects through *UV-B incidence* and *latitude*.

It can be seen that the two views, (a) and (b), lead to very similar results.


### Path analysis

Here, we take the view that case (b) holds, namely that *blue* &rarr; *n_colors*.
...
if( !all(file.exists(file_name)) )
{
  ...
  d_colors_lavaan <- d_colors[,c("glottocode", "glottocode_family", "macroarea", 
                                 "exists_blue", "n_colors_r", "UVB_r", "latitude_r", "dist2lakes_r", "clim_PC1_r", "subsistence", "log_popSize")];
  names(d_colors_lavaan) <- c("glottocode", "glottocode_family", "macroarea", 
                                 "blue", "ncol", "uvb", "lat", "d2l", "clim", "subs", "psiz");
...
semPaths(sem_blue$model, what="est", edge.label.cex=0.75, fade=FALSE, nCharNodes=0, posCol="red", negCol="blue", layout="tree2");
```

```{r fig.cap=capFig('The path model with standardized coefficients; conventions as above.'), fig.width=15, fig.height=10}
semPaths(sem_blue$model, what="std", edge.label.cex=0.75, fade=FALSE, nCharNodes=0, posCol="red", negCol="blue", layout="tree2");
```
...
lavaanPlot(model=sem_blue$model, coefs=TRUE, sig=1.00, stand=FALSE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
```
...
lavaanPlot(model=sem_blue$model, coefs=TRUE, sig=0.05, stand=TRUE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
```

This model fits the data well[^interpret_sem]: `r sprintf("*&chi;*^2^(%d) = %.1f, *p* = %.3g; CFI = %.3f, TLI = %.3f, NNFI = %.3f, RMSEA = %.3f 90%%CI [%.3f, %.3f]", fitMeasures(sem_blue$model, c("df")), fitMeasures(sem_blue$model, c("chisq")), fitMeasures(sem_blue$model, c("pvalue")), fitMeasures(sem_blue$model, c("cfi")), fitMeasures(sem_blue$model, c("tli")), fitMeasures(sem_blue$model, c("nnfi")), fitMeasures(sem_blue$model, c("rmsea")), fitMeasures(sem_blue$model, c("rmsea.ci.lower")), fitMeasures(sem_blue$model, c("rmsea.ci.upper")))`.
...
- *latitude* seems to have a direct influence on all the other measures: not only on *UV-B* (-), *blue* (-) and *n_colors* (-), but also on *climate PC1* (+), *distance to lakes* (-), and even *subsistence* (-), the strongest influences being on *climate PC1*, *UV-B* and *n_colors*. While its effects on *climate PC1* and *UV-B* (and, to a certain degree, *distance to lakes* and *subsistence*) are expected, its direct effects on *blue* and *n_colors* suggests that there are some other factors we did not model;
- *blue* is affected by *UV-B* (-), *latitude* (-), *distance to lakes* (-) and *subsistence* (+), the first strongly supporting the hypothesis of a physiological effect of UV-B incidence, and the latter supporting the idea that "cultural complexity" promotes more complex color vocabularies; intriguingly, the direct influence of *distance to lakes* is supplemented by indirect influences through *UV-B* incidence and *subsistence*;
- *n_colors* is affected by *latitude* (-), *UV-B* (-) and *blue* (+), the last supporting the stages theory, and the second suggesting that UV-B incidence might have more negative effects on color perception than just those involving "blue"; interestingly, *climate* and *subsistence* do not directly influence *n_colors*, the first in any way, while the second only through *blue*;
- *climate* influences only *UV-B* incidence (-), which may make physical sense;
- *subsistence* only influences *blue* (+).

We did not include *population size* in this model as it was highly related to *subsistence* (`r sprintf("Pearson's *r* = %.2f, *p* = %.3g; *t*(%.1f) = %.2f, mean(HG) = %.1f, mean(AGR) = %.1f, *p* = %.3g", (tmp <- cor.test(sem_blue$data$psiz, as.numeric(sem_blue$data$subs == "AGR")))$estimate, tmp$p.value, (tmp <- t.test(sem_blue$data$psiz ~ sem_blue$data$subs))$parameter, tmp$statistic, tmp$estimate["mean in group HG"], tmp$estimate["mean in group AGR"], tmp$p.value)`), as expected, and did not bring any new insights relevant to our questions.
Likewise, we did include *longitude* in our model (in order to better model the geographical patterning of linguistic diversity), but it did not seem to add anything.

Thus, this path analysis model, despite its caveats (no modeling of measurement error, no modeling of the hierarchical structure due to language families and macroareas), does support the hypothesis of an important influence of UV-B incidence on the existence of a dedicated word for "blue" above and beyond other factors (such as climate and subsistence strategy), but also reveals other influences on the color vocabulary.
...
We check how good are various techniques at predicting the existence of a dedicated word for "blue" from a collection of potential predictors; in particular, we test how the inclusion or not of the number of color words affects this prediction.
On the one hand, we estimate this when using the full dataset (i.e., we use the full data both for fitting the model and for computing various measures of goodness of prediction), but also on how well these models generalize by repeatedly generating random training and testing subsets (containing 80% and 20% of the data, respectively; we stratify by macroarea and, except for Baysian mixed-effects regressions, we do not use the language family; please note that random forests automatically sample the data).
...
We used Bayesian mixed-effects logistic regression to predict the existence of a specific word for "blue" (*blue*) from all the potential predictors, as well as when excluding the number of colors *n_colors*:
...
if( !all(file.exists(file_name)) )
{
  ...
  brms_blue__ncol <- brm(exists_blue ~ 1 + 
                           # the languages:
                         ...
                         + n_colors_r                                                  # number of color terms
                         ...
                         sample_prior=TRUE,  # needed for hypotheses tests
                         cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10));
  summary(brms_blue__ncol); mcmc_plot(brms_blue__ncol, type="trace"); mcmc_plot(brms_blue__ncol);
  (hdi_full <- hdi(brms_blue__ncol, ci=0.95)); (rope_full <- rope(brms_blue__ncol, ci=0.95));
  brms_blue__ncol <- brms_fit_indices(brms_blue__ncol);
  (cmp_null_full <- brms_compare_models(b_0__blue, brms_blue__ncol, "null", "full"));
  # Iterative manual simplification:
  brms_blue__ncol_1 <- update(brms_blue__ncol, . ~ . - hum_IQR_family - hum_median_family - hum_IQR - hum_median - clim_PC2_r - dist2water_family_r -
                                clim_PC2_family_r - latitude_r - UVB_family_r - dist2water_r - dist2rivers_r - elevation_r - elevation_family_r -
                                dist2ocean_family_r - dist2ocean_r - dist2rivers_family_r - log_popSize - dist2lakes_family_r - clim_PC1_r - UVB_r - latitude_family_r - 
                                longitude_family_r - clim_PC3_r - subsistence,
  ...
  summary(brms_blue__ncol_1); mcmc_plot(brms_blue__ncol_1, type="trace"); mcmc_plot(brms_blue__ncol_1);
  (hdi_reduced <- hdi(brms_blue__ncol_1, ci=0.95)); (rope_reduced <- rope(brms_blue__ncol_1, ci=0.95)); (fixef_reduced <- fixef(brms_blue__ncol_1));
  brms_blue__ncol_1 <- brms_fit_indices(brms_blue__ncol_1);
  # Model comparison and performance:
  (cmp_full_reduced <- brms_compare_models(brms_blue__ncol, brms_blue__ncol_1, "full", "reduced")); # reduced is better
  (cmp_null_reduced <- brms_compare_models(b_0__blue, brms_blue__ncol_1, "null", "reduced")); # reduced is better
  (cm_full    <- confusionMatrix(d$exists_blue, factor(ifelse(predict(brms_blue__ncol)[,"Estimate"] < 0.5, "no", "yes"), levels=c("no", "yes")), positive="yes"));
  (cm_reduced <- confusionMatrix(d$exists_blue, factor(ifelse(predict(brms_blue__ncol_1)[,"Estimate"] < 0.5, "no", "yes"), levels=c("no", "yes")), positive="yes"));
  # Save:
  brms_blue$with_ncolors <- list("full"   =list("cmp_2_null"=cmp_null_full, 
  ...
  # ...and without n_colors:
  brms_blue__noncol <- brm(exists_blue ~ 1 + 
                           # the languages:
                           + UVB_r                                                       # UV-B incidence
                         + longitude_r + latitude_r                                    # geographic location
                         + subsistence  + log_popSize                                  # subsistence strategy and population size
                         ...
                         sample_prior=TRUE,  # needed for hypotheses tests
                         cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10));
  summary(brms_blue__noncol); mcmc_plot(brms_blue__noncol, type="trace"); mcmc_plot(brms_blue__noncol);
  (hdi_full <- hdi(brms_blue__noncol, ci=0.95)); (rope_full <- rope(brms_blue__noncol, ci=0.95));
  brms_blue__noncol <- brms_fit_indices(brms_blue__noncol);
  (cmp_null_full <- brms_compare_models(b_0__blue, brms_blue__noncol, "null", "full"));
  # Iterative manual simplification:
  brms_blue__noncol_1 <- update(brms_blue__noncol, . ~ . - hum_IQR_family - hum_IQR - hum_median_family - dist2water_r - dist2water_family_r - clim_PC1_r - clim_PC1_family_r - 
                                  elevation_r - dist2rivers_r - hum_median - dist2ocean_r - elevation_family_r - dist2rivers_family_r - latitude_family_r - dist2lakes_family_r - 
                                  latitude_r - longitude_family_r - longitude_r - UVB_family_r - clim_PC2_r - clim_PC2_family_r - dist2ocean_family_r - subsistence,
                              save_all_pars=TRUE, sample_prior=TRUE, cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10)); 
  summary(brms_blue__noncol_1); mcmc_plot(brms_blue__noncol_1, type="trace"); mcmc_plot(brms_blue__noncol_1);
  (hdi_reduced <- hdi(brms_blue__noncol_1, ci=0.95)); (rope_reduced <- rope(brms_blue__noncol_1, ci=0.95)); (fixef_reduced <- fixef(brms_blue__noncol_1));
  brms_blue__noncol_1 <- brms_fit_indices(brms_blue__noncol_1);
  # Model comparison and performance:
  (cmp_full_reduced <- brms_compare_models(brms_blue__noncol, brms_blue__noncol_1, "full", "reduced")); # reduced is better
  (cmp_null_reduced <- brms_compare_models(b_0__blue, brms_blue__noncol_1, "null", "reduced")); # reduced is better
  (cm_full    <- confusionMatrix(d$exists_blue, factor(ifelse(predict(brms_blue__noncol)[,"Estimate"] < 0.5, "no", "yes"), levels=c("no", "yes")), positive="yes"));
  (cm_reduced <- confusionMatrix(d$exists_blue, factor(ifelse(predict(brms_blue__noncol_1)[,"Estimate"] < 0.5, "no", "yes"), levels=c("no", "yes")), positive="yes"));
  # Save:
  brms_blue$without_ncolors <- list("full"   =list("cmp_2_null"=cmp_null_full, 
                                                   "HDI"=hdi_full, "ROPE"=rope_full, "confusionMatrix"=cm_full), 
                                    "reduced"=list("cmp_2_null"=cmp_null_reduced, "cmp_2_full"=cmp_full_reduced, 
                                                   "fixef"=fixef_reduced, "HDI"=hdi_reduced, "ROPE"=rope_reduced, "confusionMatrix"=cm_reduced));
  ...
  results <- pblapply(1:n_train, function(i) # very computationally expensive
    {
      ...
      invisible(capture.output(brms_ncols   <- update(brms_blue__ncol, newdata=data_train, refresh=0, 
      ...
      invisible(capture.output(brms_noncols <- update(brms_blue__noncol, newdata=data_train, refresh=0, 
                                                      save_all_pars=TRUE, sample_prior=TRUE, cores=brms_ncores, 
                                                      iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10)),
                               type="message")); # capture the messages printed by Stan
      
      # confusion matrices:
      cm_brms_ncols   <- confusionMatrix(data_test$exists_blue, factor(ifelse(predict(brms_ncols,   newdata=data_test, allow_new_levels=TRUE)[,"Estimate"] < 0.5, 
                                                                            "no", "yes"), levels=c("no", "yes")), positive="yes");
      cm_brms_noncols <- confusionMatrix(data_test$exists_blue, factor(ifelse(predict(brms_noncols, newdata=data_test, allow_new_levels=TRUE)[,"Estimate"] < 0.5, 
                                                                            "no", "yes"), levels=c("no", "yes")), positive="yes");

      # predictors:
      hdi_ncols   <- hdi(brms_ncols,   ci=0.95); rope_ncols   <- rope(brms_ncols,   ci=0.95, verbose=FALSE);
      hdi_noncols <- hdi(brms_noncols, ci=0.95); rope_noncols <- rope(brms_noncols, ci=0.95, verbose=FALSE);
      
      # return the results:
      success_ncol <- data.frame("replication"  =i, # replication
                                 "accuracy"     =cm_brms_ncols$overall["Accuracy"],
                                 "sensitivity"  =cm_brms_ncols$byClass["Sensitivity"], 
                                 "specificity"  =cm_brms_ncols$byClass["Specificity"],
                                 "precision"    =cm_brms_ncols$byClass["Precision"],
                                 "recall"       =cm_brms_ncols$byClass["Recall"],
                                 row.names=NULL);
      preds_ncol <- data.frame("replication"=i, # replication,
                               "predictor"  =rownames(fixef(brms_ncols)), 
                               "estimate"   =fixef(brms_ncols)[,"Estimate"],
                               as.data.frame(hdi_ncols)[,c("CI_low", "CI_high")],
                               as.data.frame(rope_ncols)[,c("ROPE_low", "ROPE_high", "ROPE_Percentage")],
                               row.names=NULL);
      success_noncol <- data.frame("replication"  =i, # replication
                                   "accuracy"     =cm_brms_noncols$overall["Accuracy"],
                                   "sensitivity"  =cm_brms_noncols$byClass["Sensitivity"], 
                                   "specificity"  =cm_brms_noncols$byClass["Specificity"],
                                   "precision"    =cm_brms_noncols$byClass["Precision"],
                                   "recall"       =cm_brms_noncols$byClass["Recall"],
                                   row.names=NULL);
      preds_noncol <- data.frame("replication"=i, # replication
                                 "predictor"  =rownames(fixef(brms_noncols)), 
                                 "estimate"   =fixef(brms_noncols)[,"Estimate"],
                                 as.data.frame(hdi_noncols)[,c("CI_low", "CI_high")],
                                 as.data.frame(rope_noncols)[,c("ROPE_low", "ROPE_high", "ROPE_Percentage")],
                                 row.names=NULL);
      ...
      return (list("replication"       =i,
                   ...
                   "with_ncol"         =list("success"=success_ncol,   "predictors"=preds_ncol),
                   "without_ncol"      =list("success"=success_noncol, "predictors"=preds_noncol)));
    });
  
  # Assemble the various results and save them:
  brms_blue$training_testing <- list("replications"   =n_train,
                                     "with_ncolors"   =list("success"   =do.call(rbind, lapply(results, function(x) x$with_ncol$success)),   
                                                            "predictors"=do.call(rbind, lapply(results, function(x) x$with_ncol$predictors))), 
                                     "without_ncolors"=list("success"   =do.call(rbind, lapply(results, function(x) x$without_ncol$success)), 
                                                            "predictors"=do.call(rbind, lapply(results, function(x) x$without_ncol$predictors))));
...
On the full dataset, Bayesian mixed effects logistic regression with family and macroarea as random effects and using all potential predictors (including and excluding the number of colors words, respectively) as fixed effects, fits the data very well: 

```{r}
kable(data.frame("Predictors" =c("with n_colors", "without n_colors"),
                 "Accuracy"   =100*c(brms_blue$with_ncolors$full$confusionMatrix$overall["Accuracy"],    brms_blue$without_ncolors$full$confusionMatrix$overall["Accuracy"]),
                 "Sensitivity"=100*c(brms_blue$with_ncolors$full$confusionMatrix$byClass["Sensitivity"], brms_blue$without_ncolors$full$confusionMatrix$byClass["Sensitivity"]), 
                 "Specificity"=100*c(brms_blue$with_ncolors$full$confusionMatrix$byClass["Specificity"], brms_blue$without_ncolors$full$confusionMatrix$byClass["Specificity"]),
                 "Precision"  =100*c(brms_blue$with_ncolors$full$confusionMatrix$byClass["Precision"],   brms_blue$without_ncolors$full$confusionMatrix$byClass["Precision"]),
                 "Recall"     =100*c(brms_blue$with_ncolors$full$confusionMatrix$byClass["Recall"],      brms_blue$without_ncolors$full$confusionMatrix$byClass["Recall"])),
      row.names=FALSE, digits=c(NA, 1, 1, 1, 1, 1), 
      caption=capTab("Success (as %) on the full dataset using Bayesian mixed effects logistic regressions with all potential predictors including and excluding the number of colors words, respectively."));
```
...
```{r}
kable((tmp_all <- 
         data.frame("Predictors" =c("with n_colors", "without n_colors"),
                 "Accuracy"   =100*c(brms_blue$with_ncolors$reduced$confusionMatrix$overall["Accuracy"],    brms_blue$without_ncolors$reduced$confusionMatrix$overall["Accuracy"]),
                 "Sensitivity"=100*c(brms_blue$with_ncolors$reduced$confusionMatrix$byClass["Sensitivity"], brms_blue$without_ncolors$reduced$confusionMatrix$byClass["Sensitivity"]), 
                 "Specificity"=100*c(brms_blue$with_ncolors$reduced$confusionMatrix$byClass["Specificity"], brms_blue$without_ncolors$reduced$confusionMatrix$byClass["Specificity"]),
                 "Precision"  =100*c(brms_blue$with_ncolors$reduced$confusionMatrix$byClass["Precision"],   brms_blue$without_ncolors$reduced$confusionMatrix$byClass["Precision"]),
                 "Recall"     =100*c(brms_blue$with_ncolors$reduced$confusionMatrix$byClass["Recall"],      brms_blue$without_ncolors$reduced$confusionMatrix$byClass["Recall"]))), 
      row.names=FALSE, digits=c(NA, 1, 1, 1, 1, 1), 
      caption=capTab("Success (as %) on the full dataset using Bayesian mixed effects logistic regressions with keeping only the predictors significantly contributing, including and excluding the number of colors words, respectively."));
```

In this case, the retained predictors are:

```{r}
kable(data.frame("predictor"  =rownames(brms_blue$with_ncolors$reduced$fixef), 
                 "estimate"   =round(brms_blue$with_ncolors$reduced$fixef[,"Estimate"],2),
                 "hdi"=paste0("[", round(brms_blue$with_ncolors$reduced$HDI[,"CI_low"],2), ",", round(brms_blue$with_ncolors$reduced$HDI[,"CI_high"],2), "]"),
                 "p.rope"=sprintf("%.2g",brms_blue$with_ncolors$reduced$ROPE[,"ROPE_Percentage"]),
                 row.names=NULL), 
      row.names=FALSE, col.names=c("Predictor", "*&beta;* (estimate)", "*&beta;* (95% HDI)", "*p*~ROPE~"),
      caption=capTab(paste0("Retained predictors for Bayesian mixed effects logistic regressions following iterative maual simplification on the full dataset, starting by including the number of color words. The ROPE is [", round(brms_blue$with_ncolors$reduced$ROPE[1,"ROPE_low"],2), ",", round(brms_blue$with_ncolors$reduced$ROPE[1,"ROPE_high"],2), "].")));
```

```{r}
kable(data.frame("predictor"  =rownames(brms_blue$without_ncolors$reduced$fixef), 
                 "estimate"   =round(brms_blue$without_ncolors$reduced$fixef[,"Estimate"],2),
                 "hdi"=paste0("[", round(brms_blue$without_ncolors$reduced$HDI[,"CI_low"],2), ",", round(brms_blue$without_ncolors$reduced$HDI[,"CI_high"],2), "]"),
                 "p.rope"=sprintf("%.2g",brms_blue$without_ncolors$reduced$ROPE[,"ROPE_Percentage"]),
                 row.names=NULL), 
      row.names=FALSE, col.names=c("Predictor", "*&beta;* (estimate)", "*&beta;* (95% HDI)", "*p*~ROPE~"),
      caption=capTab(paste0("Retained predictors for Bayesian mixed effects logistic regressions following iterative maual simplification on the full dataset, starting by excluding the number of color words. The ROPE is [", round(brms_blue$without_ncolors$reduced$ROPE[1,"ROPE_low"],2), ",", round(brms_blue$without_ncolors$reduced$ROPE[1,"ROPE_high"],2), "].")));
```


When randomly splitting the dataset into 80% training/20% testing subsets `r brms_blue$training_testing$replications` times, using all the potential predictors (including and excluding the number of colors words, respectively), we see that:

```{r}
tmp_ncol   <- vapply(brms_blue$training_testing$with_ncolors$success[,-1],    function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp_noncol <- vapply(brms_blue$training_testing$without_ncolors$success[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp <- data.frame("with_ncols" =c("yes", "no"),
                  "accuracy"   =c(tmp_ncol[ grep("accuracy",    names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("accuracy",    names(tmp_noncol), fixed=TRUE) ]),
                  "sensitivity"=c(tmp_ncol[ grep("sensitivity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("sensitivity", names(tmp_noncol), fixed=TRUE) ]),
                  "specificity"=c(tmp_ncol[ grep("specificity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("specificity", names(tmp_noncol), fixed=TRUE) ]),
                  "precision"  =c(tmp_ncol[ grep("precision",   names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("precision",   names(tmp_noncol), fixed=TRUE) ]),
                  "recall"     =c(tmp_ncol[ grep("recall",      names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("recall",      names(tmp_noncol), fixed=TRUE) ]));
kable(tmp, row.names=FALSE, col.names=c("With *n_colors*?", "Accuracy", "Sensitivity", "Specificity", "Precision", "Recall"),
      caption=capTab(paste0("Various measures of success at predicting *blue* using Bayesian mixed effects logistic regressions splitting the dataset randomly into 80% training/20% testing subsets, ",brms_blue$training_testing$replications," times, including or not *n_colors*.")));
```

```{r fig.cap=capFig(paste0("Various measures of success using Bayesian mixed effects logistic regressions splitting the dataset randomly into 80% training/20% testing subsets, ",brms_blue$training_testing$replications," times, including or not *n_colors*. Boxplots show the spread of the traning/tesing values, while the solid horizontal lines show the values when using the full dataset.")), fig.width=1*7, fig.height=5}
ggplot(rbind(cbind(brms_blue$training_testing$with_ncolors$success, "with_ncol"="yes"),
             cbind(brms_blue$training_testing$without_ncolors$success, "with_ncol"="no")) %>% 
         reshape2::melt(id.vars=c("replication", "with_ncol"), variable.name="measure", value.name="value") %>% 
         mutate("measure"=factor(measure, levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), 
                "value"=value*100), 
       aes(x=measure, y=value, fill=with_ncol)) + 
  ...
  new_scale_fill() + 
  geom_boxplot(aes(x=measure, y=value, fill=with_ncol)) + 
  scale_fill_manual("With n_colors (train/test)?", values=c("yes"="gold", "no"="salmon")) +
  # measures on the full dataset:
  geom_rect(data=tmp_all %>% 
               mutate("with_ncol"=factor(if_else(Predictors == "with n_colors", "yes", "no")), levels=c("no", "yes")) %>%
               reshape2::melt(id.vars=c("with_ncol"), measure.vars=c("Accuracy", "Sensitivity", "Specificity", "Precision", "Recall"), 
             ...
             aes(xmin=as.numeric(measure)-0.5, xmax=as.numeric(measure)+0.5, ymin=value, ymax=value, color=with_ncol), fill=NA) +
  scale_color_manual("With n_colors (all data)?", values=c("yes"="chartreuse4", "no"="blue3")) + 
...
We used conditional inference trees (as implemented by `ctree()` in package `partykit`) to predict the existence of a specific word for "blue" (*blue*) from all the potential predictors, as well as when excluding the number of colors *n_colors*:
...
if( !all(file.exists(file_name)) )
{
  ...
  ctree_blue__ncol <- ctree(exists_blue ~ 
                              # the languages:
                              macroarea                                                   # macroarea
                            + UVB_r                                                       # UV-B incidence
                            + longitude_r + latitude_r                                    # geographic location
  ...
  # Decision tree without n_colors:
  ctree_blue__noncol <- ctree(exists_blue ~ 
                                # the languages:
                                macroarea                                                   # macroarea
                              + UVB_r                                                       # UV-B incidence
                              + longitude_r + latitude_r                                    # geographic location
  ...
  results <- pblapply(1:n_train, function(i) # very computationally expensive
    {
      ...
      ctree_ncols   <- ctree(exists_blue ~ 
                             ...
                             data = data_train, 
                             control = ctree_control(testtype = c("MonteCarlo")));
      ctree_noncol <- ctree(exists_blue ~ 
                              # the languages:
                              macroarea                                                   # macroarea
                            + UVB_r                                                       # UV-B incidence
                            + longitude_r + latitude_r                                    # geographic location
      ...
      # confusion matrices:
      cm_ctree_ncols   <- confusionMatrix(data_test$exists_blue, predict(ctree_ncols,  newdata=data_test, allow_new_levels=TRUE), positive="yes");
      cm_ctree_noncols <- confusionMatrix(data_test$exists_blue, predict(ctree_noncol, newdata=data_test, allow_new_levels=TRUE), positive="yes");

      # return the results:
      success_ncol <- data.frame("replication"  =i, # replication
                                 "accuracy"     =cm_ctree_ncols$overall["Accuracy"],
                                 "sensitivity"  =cm_ctree_ncols$byClass["Sensitivity"], 
                                 "specificity"  =cm_ctree_ncols$byClass["Specificity"],
                                 "precision"    =cm_ctree_ncols$byClass["Precision"],
                                 "recall"       =cm_ctree_ncols$byClass["Recall"],
                                 row.names=NULL);
      success_noncol <- data.frame("replication"  =i, # replication
                                   "accuracy"     =cm_ctree_noncols$overall["Accuracy"],
                                   "sensitivity"  =cm_ctree_noncols$byClass["Sensitivity"], 
                                   "specificity"  =cm_ctree_noncols$byClass["Specificity"],
                                   "precision"    =cm_ctree_noncols$byClass["Precision"],
                                   "recall"       =cm_ctree_noncols$byClass["Recall"],
                                   row.names=NULL);

      return (list("replication"       =i,
                   "data_train_indices"=train_test_splits[[i]]$in_id, 
                   "data_test_indices" =setdiff(1:nrow(train_test_splits[[i]]$data), train_test_splits[[i]]$in_id),
                   "with_ncol"         =list("success"=success_ncol),
                   "without_ncol"      =list("success"=success_noncol)));
    });
  
  
  # Save to file:
  ctree_blue <- list("data"=d,
                     "with_ncolors"=ctree_blue__ncol, 
                     "without_ncolors"=ctree_blue__noncol,
...
```{r fig.cap=capFig('Conditional inference trees for predicting *blue* using all the potential predictors (top) and when excplitiely excluding *n_colors* (bottom). Please note that some predictors are transformed (the "_r" suffix), the most important here being *n_colors_r* (0.3=5 and 0.4=6 colors).'), fig.width=10, fig.height=6, fig.show='hold', out.height="50%"}
plot(ctree_blue$with_ncolors,    gp=gpar(fontsize = 11));
plot(ctree_blue$without_ncolors, gp=gpar(fontsize = 11));
```

Using or not *n_colors* achieve comparable levels of data fit (top = with *n_colors*, bottom = without *n_colors*):

```{r}
cm1 <- confusionMatrix(ctree_blue$data$exists_blue, predict(ctree_blue$with_ncolors), positive="yes");
cm2 <- confusionMatrix(ctree_blue$data$exists_blue, predict(ctree_blue$without_ncolors), positive="yes");
kable(data.frame("Predictors" =c("with n_colors", "without n_colors"),
                 "Accuracy"   =100*c(cm1$overall["Accuracy"],    cm2$overall["Accuracy"]),
                 "Sensitivity"=100*c(cm1$byClass["Sensitivity"], cm2$byClass["Sensitivity"]), 
                 "Specificity"=100*c(cm1$byClass["Specificity"], cm2$byClass["Specificity"]),
                 "Precision"  =100*c(cm1$byClass["Precision"],   cm2$byClass["Precision"]),
                 "Recall"     =100*c(cm1$byClass["Recall"],      cm2$byClass["Recall"])),
      row.names=FALSE, digits=c(NA, 1, 1, 1, 1, 1), 
      caption=capTab("Success (as %) on the full dataset using conditional inference trees with all potential predictors including and excluding the number of colors words, respectively."));
```

It seems that the best predictor for having a word for "blue" is having > 6 color terms. 
In the absence of this predictor, there is a distinction between Eurasian and Australian languages, on the one hand, and African, Papunesian and American languages, on the other, in both regions with small languages (≤ ≈1,000,000 speakers) having less "blue"; while in Eurasia and Australia UV-N incidence has a negative effect, in Africa, Papunesia and America, it is the distance to large bodies of water (lakes and oceans) that does.


When randomly splitting the dataset into 80% training/20% testing subsets `r ctree_blue$training_testing$replications` times, using all the potential predictors (including and excluding the number of colors words, respectively), we see that:

```{r}
tmp_ncol   <- vapply(ctree_blue$training_testing$with_ncolors$success[,-1],    function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp_noncol <- vapply(ctree_blue$training_testing$without_ncolors$success[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp <- data.frame("with_ncols" =c("yes", "no"),
                  "accuracy"   =c(tmp_ncol[ grep("accuracy",    names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("accuracy",    names(tmp_noncol), fixed=TRUE) ]),
                  "sensitivity"=c(tmp_ncol[ grep("sensitivity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("sensitivity", names(tmp_noncol), fixed=TRUE) ]),
                  "specificity"=c(tmp_ncol[ grep("specificity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("specificity", names(tmp_noncol), fixed=TRUE) ]),
                  ...
                  "recall"     =c(tmp_ncol[ grep("recall",      names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("recall",      names(tmp_noncol), fixed=TRUE) ]));
kable(tmp, row.names=FALSE, col.names=c("With *n_colors*?", "Accuracy", "Sensitivity", "Specificity", "Precision", "Recall"),
      caption=capTab(paste0("Various measures of success at predicting *blue* using conditional inference trees splitting the dataset randomly into 80% training/20% testing subsets, ",ctree_blue$training_testing$replications," times, including or not *n_colors*.")));
```

```{r fig.cap=capFig(paste0("Various measures of success using conditional inference trees splitting the dataset randomly into 80% training/20% testing subsets, ",ctree_blue$training_testing$replications," times, including or not *n_colors*. Boxplots show the spread of the traning/tesing values, while the solid horizontal lines show the values when using the full dataset.")), fig.width=1*7, fig.height=5}
ggplot(rbind(cbind(ctree_blue$training_testing$with_ncolors$success, "with_ncol"="yes"),
             cbind(ctree_blue$training_testing$without_ncolors$success, "with_ncol"="no")) %>% 
         reshape2::melt(id.vars=c("replication", "with_ncol"), variable.name="measure", value.name="value") %>% 
         mutate("measure"=factor(measure, levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), 
                "value"=value*100), 
       aes(x=measure, y=value, fill=with_ncol)) + 
...
We used random forests (as implemented by `randomForest()` in package `randomForest`) and conditional random forests (as implemented by `cforest()` in package `partykit`) to predict the existence of a specific word for "blue" (*blue*) from all the potential predictors, as well as when excluding the number of colors *n_colors*:

```{r include=FALSE}
file_name <- "./cached_results/rforest_blue.RData";
if( !all(file.exists(file_name)) )
{
  # The potential predictors (excluding n_colors):
  list_predictors_noncol <- c(# the languages:
                              "macroarea",                                                       # macroarea
                              "UVB_r",                                                           # UV-B incidence
                              "longitude_r", "latitude_r",                                       # geographic location
                              "subsistence", "log_popSize",                                      # subsistence strategy and population size
                              ...
                              "dist2water_family_r", "dist2ocean_family_r", "dist2lakes_family_r", "dist2rivers_family_r");         # distances to large bodies of water 
  # ... and with n_colors:
  list_predictors_ncol <- c(list_predictors_noncol,
                            "n_colors_r"); # number of color terms
  ...
  f_ncols        <- formula(paste0("exists_blue ~ ", paste0(list_predictors_ncol,collapse=" + "))); # with n_colors
  f_noncols      <- formula(paste0("exists_blue ~ ", paste0(list_predictors_noncol,collapse=" + "))); # and without
  results <- pblapply(1:n_replications, function(i) # very computationally expensive
    {
      # fit the random forests:
      rf_ncols   <- randomForest(f_ncols, data=d, importance=TRUE);
      cf_ncols   <- cforest(f_ncols, data=d, ntree=cf_ntree);
      rf_noncols <- randomForest(f_noncols, data=d, importance=TRUE);
      cf_noncols <- cforest(f_noncols, data=d, ntree=cf_ntree);
      
      # confusion matrices:
      cm_rf_ncols   <- confusionMatrix(d$exists_blue, rf_ncols$predicted,   positive="yes"); # with n_colors
      cm_cf_ncols   <- confusionMatrix(d$exists_blue, predict(cf_ncols),    positive="yes");
      cm_rf_noncols <- confusionMatrix(d$exists_blue, rf_noncols$predicted, positive="yes"); # and without
      cm_cf_noncols <- confusionMatrix(d$exists_blue, predict(cf_noncols),  positive="yes");
      
      # predictor importance:
      pia_rf_ncols   <- importance(rf_ncols,   type=1); # accuracy-based
      pig_rf_ncols   <- importance(rf_ncols,   type=2); # gini-based
      piu_cf_ncols   <- varimp(cf_ncols, conditional=FALSE, nperm=cf_nperm); # unconditional
      pia_rf_noncols <- importance(rf_noncols, type=1); # accuracy-based
      pig_rf_noncols <- importance(rf_noncols, type=2); # gini-based
      piu_cf_noncols <- varimp(cf_noncols, conditional=FALSE, nperm=cf_nperm); # unconditional
  
      # return the results:
      # with n_colors:
      success_ncol <- data.frame("replication"     =i,                               # replication
                                 "rf_accuracy"     =cm_rf_ncols$overall["Accuracy"], # randomForest
                                 "rf_sensitivity"  =cm_rf_ncols$byClass["Sensitivity"], 
                                 "rf_specificity"  =cm_rf_ncols$byClass["Specificity"],
                                 "rf_precision"    =cm_rf_ncols$byClass["Precision"],
                                 "rf_recall"       =cm_rf_ncols$byClass["Recall"],
                                 "cf_accuracy"     =cm_cf_ncols$overall["Accuracy"], # cforest
                                 "cf_sensitivity"  =cm_cf_ncols$byClass["Sensitivity"], 
                                 "cf_specificity"  =cm_cf_ncols$byClass["Specificity"],
                                 "cf_precision"    =cm_cf_ncols$byClass["Precision"],
                                 "cf_recall"       =cm_cf_ncols$byClass["Recall"], 
                                 row.names=NULL);
      pi_df_ncol  <- data.frame("replication"      =i,                    # replication
                                "predictor"        =list_predictors_ncol, # predictors
                                "rf_accuracy_based"=pia_rf_ncols,         # randomForest accuracy-based
                                "rf_gini_based"    =pig_rf_ncols,         # randomForest gini-based
                                "cf_unconditional" =piu_cf_ncols,         # cforest unconditional
                                row.names=NULL);
      # without n_colors:
      success_noncol <- data.frame("replication"     =i,                                 # replication
                                   "rf_accuracy"     =cm_rf_noncols$overall["Accuracy"], # randomForest
                                   "rf_sensitivity"  =cm_rf_noncols$byClass["Sensitivity"], 
                                   "rf_specificity"  =cm_rf_noncols$byClass["Specificity"],
                                   "rf_precision"    =cm_rf_noncols$byClass["Precision"],
                                   "rf_recall"       =cm_rf_noncols$byClass["Recall"],
                                   "cf_accuracy"     =cm_cf_noncols$overall["Accuracy"], # cforest
                                   "cf_sensitivity"  =cm_cf_noncols$byClass["Sensitivity"], 
                                   "cf_specificity"  =cm_cf_noncols$byClass["Specificity"],
                                   "cf_precision"    =cm_cf_noncols$byClass["Precision"],
                                   "cf_recall"       =cm_cf_noncols$byClass["Recall"], 
                                   row.names=NULL);
      pi_df_noncol  <- data.frame("replication"      =i,                      # replication
                                  "predictor"        =list_predictors_noncol, # predictors
                                  "rf_accuracy_based"=pia_rf_noncols,         # randomForest accuracy-based
                                  "rf_gini_based"    =pig_rf_noncols,         # randomForest gini-based
                                  "cf_unconditional" =piu_cf_noncols,         # cforest unconditional
                                  row.names=NULL);
      
      
      return (list("replication" =i,
                   "with_ncol"   =list("success"=success_ncol,   "pred_importance"=pi_df_ncol),
                   "without_ncol"=list("success"=success_noncol, "pred_importance"=pi_df_noncol)));
  ...
  rforest_blue <- list("data"           =d,
                       ...
                       "with_ncolors"   =list("success"=do.call(rbind, lapply(results, function(x) x$with_ncol$success)),   
                                              "pred_importance"=do.call(rbind, lapply(results, function(x) x$with_ncol$pred_importance))), 
                       "without_ncolors"=list("success"=do.call(rbind, lapply(results, function(x) x$without_ncol$success)), 
                                              "pred_importance"=do.call(rbind, lapply(results, function(x) x$without_ncol$pred_importance))));
...
```{r}
tmp_ncol   <- vapply(rforest_blue$with_ncolors$success[,-1],    function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp_noncol <- vapply(rforest_blue$without_ncolors$success[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp <- data.frame("with_ncols" =rep(c("yes", "no"), each=2),
                  "method"     =rep(c("random forest", "conditional random forest"), times=2),
                  "accuracy"   =c(tmp_ncol[ grep("accuracy",    names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("accuracy",    names(tmp_noncol), fixed=TRUE) ]),
                  "sensitivity"=c(tmp_ncol[ grep("sensitivity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("sensitivity", names(tmp_noncol), fixed=TRUE) ]),
                  "specificity"=c(tmp_ncol[ grep("specificity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("specificity", names(tmp_noncol), fixed=TRUE) ]),
                  "precision"  =c(tmp_ncol[ grep("precision",   names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("precision",   names(tmp_noncol), fixed=TRUE) ]),
                  "recall"     =c(tmp_ncol[ grep("recall",      names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("recall",      names(tmp_noncol), fixed=TRUE) ]));
kable(tmp, row.names=FALSE, col.names=c("With *n_colors*?", "Method", "Accuracy", "Sensitivity", "Specificity", "Precision", "Recall"),
      caption=capTab(paste0("Various measures of success at predicting *blue* using two random forest methods, ",rforest_blue$replications," replications, and including or not *n_colors*.")));
```

```{r fig.cap=capFig(paste0("Various measures of success using (conditional) random forests with ",rforest_blue$replications," replications, including or not *n_colors*.")), fig.width=2*4, fig.height=5}
ggplot(rbind(cbind(rforest_blue$with_ncolors$success, "with_ncol"="yes"),
             cbind(rforest_blue$without_ncolors$success, "with_ncol"="no")) %>% 
         reshape2::melt(id.vars=c("replication", "with_ncol"), variable.name="measure", value.name="value") %>% 
  ...
  new_scale_fill() + 
  geom_boxplot(aes(x=measure, y=value, fill=with_ncol)) + 
  scale_fill_manual("With n_colors?", values=c("yes"="gold", "no"="salmon")) +
...
```{r fig.cap=capFig(paste0("Accuracy-based predictor importance from random forests using ",rforest_blue$replications," replications, and including or not *n_colors*: this is a measure of the amount by which removing a variable decreases the accuracy thus higher values point to more relevant predictors. Left column: direct comparison between the importance when including and when excluding *n_colors* (the predictors are ordered alphabetically from the bottom); center column: predictors ordered by their importance when including *n_colors*; right column: predictors ordered by their importance when excluding *n_colors*.")), fig.width=4*3, fig.height=7*1}
grid.arrange(ggplot(rbind(cbind(rforest_blue$with_ncolors$pred_importance, "with_ncol"="yes"), 
                          cbind(rforest_blue$without_ncolors$pred_importance, "with_ncol"="no")), 
                    aes(x=reorder(predictor, as.character(predictor), unique), y=MeanDecreaseAccuracy, fill=with_ncol)) +
               theme_bw() +
               geom_boxplot(color="black") +
               labs(x="Predictors (sorted alphabetically)", y="Mean decrease accuracy") +
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=10),
                     axis.title=element_text(size=16),
                     legend.position="bottom") +
               ggtitle("With n_colors") + 
               scale_fill_manual("With n_colors?", values=c("yes"="gold", "no"="salmon")) + 
               coord_flip() +
               NULL,
             ggplot(rforest_blue$with_ncolors$pred_importance, 
               ...
               coord_flip() + 
               NULL,
             ggplot(rforest_blue$without_ncolors$pred_importance, 
                    aes(x=reorder(predictor, MeanDecreaseAccuracy, median), y=MeanDecreaseAccuracy)) +
               theme_bw() +
               ...
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     ...
                     axis.title=element_text(size=16),
                     legend.position="none") +
               ggtitle("Without n_colors") + 
               coord_flip() + 
               NULL,
             ncol=3);
```

```{r fig.width=5, fig.cap=capFig(paste0("Gini-index-based predictor importance from random forests using ",rforest_blue$replications," replications, and including or not *n_colors*: this measures by how much the Gini impurity decreasse when a variable is chosen to split a node (please note that only the relative values matter, and that there is a bias towards using numeric variables to split nodes). Column conventions as above.")), fig.width=4*3, fig.height=7*1}
grid.arrange(ggplot(rbind(cbind(rforest_blue$with_ncolors$pred_importance, "with_ncol"="yes"), 
                          cbind(rforest_blue$without_ncolors$pred_importance, "with_ncol"="no")), 
                    aes(x=reorder(predictor, as.character(predictor), unique), y=MeanDecreaseGini, fill=with_ncol)) +
               theme_bw() +
               geom_boxplot(color="black") +
               labs(x="Predictors (sorted alphabetically)", y="Mean decrease Gini index") +
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=10),
                     axis.title=element_text(size=16),
                     legend.position="bottom") +
...
```{r fig.width=5, fig.cap=capFig(paste0("Unconditional predictor importance from conditional random forests using ",rforest_blue$replications," replications, and including or not *n_colors*: this is similar to the accuracy-based importance from random forests. Column conventions as above.")), fig.width=4*3, fig.height=7*1}
grid.arrange(ggplot(rbind(cbind(rforest_blue$with_ncolors$pred_importance, "with_ncol"="yes"), 
                          cbind(rforest_blue$without_ncolors$pred_importance, "with_ncol"="no")), 
                    aes(x=reorder(predictor, as.character(predictor), unique), y=cf_unconditional, fill=with_ncol)) +
               theme_bw() +
               geom_boxplot(color="black") +
               labs(x="Predictors (sorted alphabetically)", y="Unconditional importance") +
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=10),
                     axis.title=element_text(size=16),
                     legend.position="bottom") +
...
a) *blue* is relatively well predicted, especially when including *n_colors*, and that conditional random forests perform slightly better than "classic" random forests;
b) the various methods for estimating the predictor importance produce very similar results;
c) when included, *n_colors* is by far the most important predictor;
d) the importance of the other predictors is relatively stable with or without *n_colors*, and suggests that *population size*, *UV-B incidence*, *humidity*, *climate PC1* and *latitude* tend to be the (next) most important.


#### Support Vector Machines (SVM)

We used Support Vector Machines (SVMs, as implemented by `fit(...,model="svm")` in the `rminer` package) to predict the existence of a specific word for "blue" (*blue*) from all the potential predictors, as well as when excluding the number of colors *n_colors*.
...
if( !all(file.exists(file_name)) )
{
  ...
  d <- d[ complete.cases(d[,c("exists_blue", list_predictors_ncol)]), c("exists_blue", list_predictors_ncol)];
  # Save:
  ...
  f_noncols       <- formula(paste0("exists_blue ~ ", paste0(list_predictors_noncol,collapse=" + "))); # and without
  # fit the SVMs:
  svm_fit_ncols   <- rminer::fit(f_ncols, d, model="svm", task="class");
  svm_fit_noncols <- rminer::fit(f_noncols, d, model="svm", task="class");
  # confusion matrices on the test data:
  cm_svm_ncols   <- confusionMatrix(d$exists_blue, predict(svm_fit_ncols,   d, type='class'), positive="yes");
  cm_svm_noncols <- confusionMatrix(d$exists_blue, predict(svm_fit_noncols, d, type='class'), positive="yes");
  # predictor importance (based on sensitivity analysis):
  tmp             <- rminer::Importance(svm_fit_ncols, d, method="sensv"); 
  pia_svm_ncols   <- tmp$imp; names(pia_svm_ncols)   <- names(d); pia_svm_ncols   <- pia_svm_ncols[names(pia_svm_ncols) %in% list_predictors_ncol];
  tmp             <- rminer::Importance(svm_fit_noncols, d, method="sensv"); 
  pia_svm_noncols <- tmp$imp; names(pia_svm_noncols) <- names(d); pia_svm_noncols <- pia_svm_noncols[names(pia_svm_noncols) %in% list_predictors_noncol];
  # Save:
  svm_blue$with_ncolors <- list("success"=data.frame("accuracy"   =cm_svm_ncols$overall["Accuracy"],
                                                     "sensitivity"=cm_svm_ncols$byClass["Sensitivity"], 
                                                     "specificity"=cm_svm_ncols$byClass["Specificity"],
                                                     "precision"  =cm_svm_ncols$byClass["Precision"],
                                                     "recall"     =cm_svm_ncols$byClass["Recall"],
                                                     row.names=NULL),   
                                "pred_importance"=data.frame("predictor"  =list_predictors_ncol,                # predictors
                                                             "importance" =pia_svm_ncols[list_predictors_ncol], # sensitivity-based
                                                             row.names=NULL));
  svm_blue$without_ncolors <- list("success"=data.frame("accuracy"   =cm_svm_noncols$overall["Accuracy"],
                                                        "sensitivity"=cm_svm_noncols$byClass["Sensitivity"], 
                                                        "specificity"=cm_svm_noncols$byClass["Specificity"],
                                                        "precision"  =cm_svm_noncols$byClass["Precision"],
                                                        "recall"     =cm_svm_noncols$byClass["Recall"],
                                                        row.names=NULL),   
                                   "pred_importance"=data.frame("predictor"  =list_predictors_noncol,                # predictors
                                                                "importance" =pia_svm_noncols[list_predictors_noncol], # sensitivity-based
                                                                row.names=NULL));
  ...
  results <- pblapply(1:n_train, function(i)
    {
      ...
      data_train <- data_train[ complete.cases(data_train[,c("exists_blue", list_predictors_ncol)]), c("exists_blue", list_predictors_ncol)];
      data_test  <- data_test [ complete.cases(data_test [,c("exists_blue", list_predictors_ncol)]), c("exists_blue", list_predictors_ncol)];
      
      # fit the SVMs:
      svm_fit_ncols   <- rminer::fit(f_ncols, data_train, model="svm", task="class");
      svm_fit_noncols <- rminer::fit(f_noncols, data_train, model="svm", task="class");

      # confusion matrices on the test data:
      cm_svm_ncols   <- confusionMatrix(data_test$exists_blue, predict(svm_fit_ncols,   data_test, type='class'), positive="yes");
      cm_svm_noncols <- confusionMatrix(data_test$exists_blue, predict(svm_fit_noncols, data_test, type='class'), positive="yes");
    
      # predictor importance (based on sensitivity analysis):
      tmp             <- rminer::Importance(svm_fit_ncols, data_train, method="sensv"); 
      pia_svm_ncols   <- tmp$imp; names(pia_svm_ncols)   <- names(data_train); pia_svm_ncols   <- pia_svm_ncols[names(pia_svm_ncols) %in% list_predictors_ncol];
      tmp             <- rminer::Importance(svm_fit_noncols, data_train, method="sensv"); 
      pia_svm_noncols <- tmp$imp; names(pia_svm_noncols) <- names(data_train); pia_svm_noncols <- pia_svm_noncols[names(pia_svm_noncols) %in% list_predictors_noncol];

      # return the results:
      # with n_colors:
      success_ncol <- data.frame("replication"    =i, # replication
                                 "accuracy"   =cm_svm_ncols$overall["Accuracy"],
                                 "sensitivity"=cm_svm_ncols$byClass["Sensitivity"], 
                                 "specificity"=cm_svm_ncols$byClass["Specificity"],
                                 "precision"  =cm_svm_ncols$byClass["Precision"],
                                 "recall"     =cm_svm_ncols$byClass["Recall"],
      ...
      # without n_colors:
      success_noncol <- data.frame("replication"    =i, # replication
                                   "accuracy"   =cm_svm_noncols$overall["Accuracy"],
                                   "sensitivity"=cm_svm_noncols$byClass["Sensitivity"], 
                                   "specificity"=cm_svm_noncols$byClass["Specificity"],
                                   "precision"  =cm_svm_noncols$byClass["Precision"],
                                   "recall"     =cm_svm_noncols$byClass["Recall"],
...
```{r}
kable((tmp_all <- data.frame("Predictors" =c("with n_colors", "without n_colors"),
                 rbind(100*svm_blue$with_ncolors$success, 100*svm_blue$without_ncolors$success))),
      row.names=FALSE, digits=c(NA, 1, 1, 1, 1, 1), 
      caption=capTab("Success (as %) on the full dataset using SVMs with all potential predictors including and excluding the number of colors words, respectively."));
```

The predictors ordered by their importance (only those with importance > 0):

```{r results='asis'}
tmp <- svm_blue$with_ncolors$pred_importance[ order(svm_blue$with_ncolors$pred_importance$importance, decreasing=TRUE), ];
tmp <- tmp[ tmp$importance > 0, ];
cat(paste0("- **with** *n_colors*: ", paste0("*",tmp$predictor,"*", " (", round(tmp$importance,2), ")", collapse=", ")));

cat("\n");
tmp <- svm_blue$without_ncolors$pred_importance[ order(svm_blue$without_ncolors$pred_importance$importance, decreasing=TRUE), ];
tmp <- tmp[ tmp$importance > 0, ];
cat(paste0("- **without** *n_colors*: ", paste0("*",tmp$predictor,"*", " (", round(tmp$importance,2), ")", collapse=", ")));
```
...
```{r}
tmp_ncol   <- vapply(svm_blue$training_testing$with_ncolors$success[,-1],    function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp_noncol <- vapply(svm_blue$training_testing$without_ncolors$success[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1));
tmp <- data.frame("with_ncols" =rep(c("yes", "no")),
                  "accuracy"   =c(tmp_ncol[ grep("accuracy",    names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("accuracy",    names(tmp_noncol), fixed=TRUE) ]),
                  "sensitivity"=c(tmp_ncol[ grep("sensitivity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("sensitivity", names(tmp_noncol), fixed=TRUE) ]),
                  "specificity"=c(tmp_ncol[ grep("specificity", names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("specificity", names(tmp_noncol), fixed=TRUE) ]),
                  "precision"  =c(tmp_ncol[ grep("precision",   names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("precision",   names(tmp_noncol), fixed=TRUE) ]),
                  "recall"     =c(tmp_ncol[ grep("recall",      names(tmp_ncol), fixed=TRUE) ], tmp_noncol[ grep("recall",      names(tmp_noncol), fixed=TRUE) ]));
kable(tmp, row.names=FALSE, col.names=c("With *n_colors*?", "Accuracy", "Sensitivity", "Specificity", "Precision", "Recall"),
      caption=capTab(paste0("Various measures of success at predicting *blue* using SVMs with ",svm_blue$training_testing$replications," training/testing sets, and including or not *n_colors*.")));
```

```{r fig.cap=capFig(paste0("Various measures of success using SVMs with ",svm_blue$replications," training/testing sets, including or not *n_colors*.")), fig.width=1*6, fig.height=5}
ggplot(rbind(cbind(svm_blue$training_testing$with_ncolors$success, "with_ncol"="yes"),
             cbind(svm_blue$training_testing$without_ncolors$success, "with_ncol"="no")) %>% 
         reshape2::melt(id.vars=c("replication", "with_ncol"), variable.name="measure", value.name="value") %>% 
         mutate("measure"=factor(measure, levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), 
                "value"=value*100), 
       aes(x=measure, y=value, fill=with_ncol)) + 
  ...
  geom_rect(data=tmp_all %>% 
               mutate("with_ncol"=factor(if_else(Predictors == "with n_colors", "yes", "no")), levels=c("no", "yes")) %>%
               reshape2::melt(id.vars=c("with_ncol"), 
                              measure.vars=c("accuracy", "sensitivity", "specificity", "precision", "recall"), variable.name="measure", value.name="value") %>% 
               mutate("measure"=factor(measure, 
                                       levels=c("accuracy", "sensitivity", "specificity", "precision", "recall"))), 
             aes(xmin=as.numeric(measure)-0.5, xmax=as.numeric(measure)+0.5, ymin=value, ymax=value, color=with_ncol), fill=NA) +
  scale_color_manual("With n_colors (all data)?", values=c("yes"="chartreuse4", "no"="blue3")) + 
...
```{r fig.cap=capFig(paste0("Specificity-based predictor importance from SVMs using ",svm_blue$training_testing$replications," traning/testing sets, and including or not *n_colors*.")), fig.width=4*3, fig.height=7*1}
grid.arrange(ggplot(rbind(cbind(svm_blue$training_testing$with_ncolors$pred_importance, "with_ncol"="yes"), 
                          cbind(svm_blue$training_testing$without_ncolors$pred_importance, "with_ncol"="no")), 
                    aes(x=reorder(predictor, as.character(predictor), unique), y=importance, fill=with_ncol)) +
               theme_bw() +
               geom_boxplot(color="black") +
               labs(x="Predictors (sorted alphabetically)", y="Importance") +
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=10),
                     axis.title=element_text(size=16),
                     legend.position="bottom") +
               ...
               coord_flip() +
               NULL,
             ggplot(svm_blue$training_testing$with_ncolors$pred_importance, 
               ...
               coord_flip() + 
               NULL,
             ggplot(svm_blue$training_testing$without_ncolors$pred_importance, 
                    aes(x=reorder(predictor, importance, median), y=importance)) +
               theme_bw() +
               geom_boxplot(fill="salmon", color="black") +
               labs(x="Predictors (sorted by decreasing importance)", y="Importance") +
...
a) *blue* is relatively well predicted, especially when including *n_colors*;
b) when included, *n_colors* is the most important predictor, but not exceedingly better than the next ones;
c) the importance of the other predictors changes quite a bit with or without *n_colors*:
  - with *n_colors*, the next best are: *population size*, *distance to lakes*, *climate PC3* and *distance to oceans for the families*,
  - without *n_colors*, the best are: *population size*, *distance to oceans for the families*, *humidity*, *distance to lakes*, *climate PC1*, *PC2* and *PC3* and *UV-B incidence*.
...
- all techniques predict *blue* very well from the available information, with very little difference between them (in fact, Bayesian mixed effects logistic regression tends to be the best)
- the performance is slightly higher when including *n_colors* among the predictors
- consistently, the most relevant predictors are: *n_colors*, *UV-B incidence*, *population size*, *climate*/*humidity* and *distance to large bodies of standing water* (mostly lakes but also sometimes seas/oceans).

In what concerns generalizatibility (either when explicitly repeatedly splitting the data into training/testing subsets or internally using random forests):
...
Thus, the presence of a dedicated word for "blue" is in large part explained by the number of color words and a few environmental variables concerning UV-B radiation, climate and distance to large bodies of standing water.
...
Moreover, in support of proposals such as @berlin_basic_1991, the number of color categories present in a language is very strongly positively linked with the presence of a dedicated word for blue, with 5-6 categories as an approximate threshold for its emergence, but it is currently unclear what the precise causal relationship between the two is.
Interestingly, different techniques vary in their support for an influence of measures of "cultural complexity" (such as subsistence strategy and population size), but there is at least a hint of a positive relationship.
...
```{r fig.cap=capFig("Graphical representation of the full Hypothesis 2 with our measures as a SEM diagram."), include=FALSE, eval=FALSE}
DiagrammeR::grViz(paste0('
  ...
  ncol [label = "n_colors", tooltip = "n_colors"]; 
  lat  [label = "latitude", tooltip = "latitude"]; 
  clim [label = "climate PC1", tooltip = "climate PC1"]; 
...
#### # color categories &rarr; daltonism

```{r fig.cap=capFig('Population frequency of daltonism (in %) function of the number of color categories, showing the data points and the linear and LOESS trends (with 95% CIs).'), fig.height=1*5, fig.width=1*5}
ggplot(d_colors, aes(y=daltonism, x=n_colors)) + 
  geom_point(alpha=0.35) + 
  geom_smooth(method="lm", color="blue") + geom_smooth(method="loess", color="red") + 
  theme_bw() +
  labs(x="Number of color categories", y="Population frequency of datonism (%)") +
  theme(axis.text.x = element_text(size =14, face = "bold"), 
        axis.text.y = element_text(hjust = 1, size =14, face = "bold"),
        axis.title=element_text(size=14),
        legend.title=element_text(size =16),
...
```{r include=FALSE}
# n_colors -> daltonism:
if( !all(file.exists("./cached_results/b_ncolors__daltonism.RData")) )
{
  # Fit the model:
  b_ncolors__daltonism <- brm(daltonism_r ~ 1 + n_colors_r + I(n_colors_r^2)  + (1 | glottocode_family) + (1 | macroarea), # n_colors -> daltonism_r
                              family=Beta(), data=d_colors, 
                              prior=c(prior(normal(0, 5), class="Intercept"),
                                      prior(normal(0, 5), class="b")), # pretty wide priors centered on 0
                              save_all_pars=TRUE, # needed for Bayes factors
                              sample_prior=TRUE,  # needed for hypotheses tests
                              cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_ncolors__daltonism <- brms_fit_indices(b_ncolors__daltonism);
  summary(b_ncolors__daltonism); mcmc_plot(b_ncolors__daltonism, type="trace"); mcmc_plot(b_ncolors__daltonism);
  (h_ncolors__daltonism <- brms::hypothesis(b_ncolors__daltonism, c("n_colors_r = 0", "In_colors_rE2 = 0")));
  (hdi_ncolors__daltonism <- hdi(b_ncolors__daltonism, ci=0.95));
  compare_0_ncolors__daltonism <- brms_compare_models(b_0__daltonism, b_ncolors__daltonism, "null", "+ n_colors"); # no effect of n_colors...
  
  # Try to remove the quadratic effect:
  b_ncolors__daltonism_1 <- update(b_ncolors__daltonism, . ~ . - I(n_colors_r^2), 
                                   save_all_pars=TRUE, sample_prior=TRUE, cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_ncolors__daltonism_1 <- brms_fit_indices(b_ncolors__daltonism_1);
  summary(b_ncolors__daltonism_1); mcmc_plot(b_ncolors__daltonism_1, type="trace"); mcmc_plot(b_ncolors__daltonism_1);
  (h_ncolors__daltonism_1 <- brms::hypothesis(b_ncolors__daltonism_1, c("n_colors_r = 0")));
  (hdi_ncolors__daltonism_1 <- hdi(b_ncolors__daltonism_1, ci=0.95));
  compare_ncolors__daltonism_1 <- brms_compare_models(b_ncolors__daltonism, b_ncolors__daltonism_1, "full", "- qudratic"); # quadratic effect does not matter
  b_ncolors__daltonism <- b_ncolors__daltonism_1; h_ncolors__daltonism <- h_ncolors__daltonism_1; hdi_ncolors__daltonism <- hdi_ncolors__daltonism_1; # keep the linear only
  
  # Compare with null:
  compare_0_ncolors__daltonism <- brms_compare_models(b_0__daltonism, b_ncolors__daltonism, "null", "+ n_colors"); # no effect of n_colors...
  
  # Save results:
  save(compare_0_ncolors__daltonism, h_ncolors__daltonism, hdi_ncolors__daltonism, #b_ncolors__daltonism, # no need to save the actual model
       file="./cached_results/b_ncolors__daltonism.RData", compress="xz", compression_level=9);
} else
{
  load("./cached_results/b_ncolors__daltonism.RData");
}
```

The number of colors categories (*n_colors*) has no (or very weakly positive) effect on *daltonism*: *&beta;~n_colors~* = `r round(h_ncolors__daltonism$hypothesis$Estimate[1],2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_ncolors__daltonism$CI_low[ hdi_ncolors__daltonism$Parameter == "b_n_colors_r" ], hdi_ncolors__daltonism$CI_high[ hdi_ncolors__daltonism$Parameter == "b_n_colors_r" ])`), *p*(*&beta;*=0) = `r sprintf("%.2g",h_ncolors__daltonism$hypothesis$Post.Prob[1])`); and fits the data comparable to the null model (Bayes factor = `r sprintf("%.2g",compare_0_ncolors__daltonism$BF)`, LOO = `r sprintf("%.2f [SE=%.2f]", compare_0_ncolors__daltonism$LOO["null", "elpd_diff"], compare_0_ncolors__daltonism$LOO["null", "se_diff"])`, WAIC = `r sprintf("%.2f [SE=%.2f]", compare_0_ncolors__daltonism$WAIC["null", "elpd_diff"], compare_0_ncolors__daltonism$WAIC["null", "se_diff"])`, K-fold = `r sprintf("%.2f [SE=%.2f]", compare_0_ncolors__daltonism$KFOLD["null", "elpd_diff"], compare_0_ncolors__daltonism$KFOLD["null", "se_diff"])`).


#### daltonism &rarr; # color categories 

```{r fig.cap=capFig('Population frequency of daltonism (in %) affecting the number of color categories, showing the data points and the linear and LOESS trends (with 95% CIs).'), fig.height=1*5, fig.width=1*5}
ggplot(d_colors, aes(x=daltonism, y=n_colors)) + 
  geom_point(alpha=0.35) + 
  geom_smooth(method="lm", color="blue") + geom_smooth(method="loess", color="red") + 
  theme_bw() +
  labs(y="Number of color categories", x="Population frequency of datonism (%)") +
  theme(axis.text.x = element_text(size =14, face = "bold"), 
        axis.text.y = element_text(hjust = 1, size =14, face = "bold"),
        axis.title=element_text(size=14),
        legend.title=element_text(size =16),
...
```{r include=FALSE}
# daltonism -> n_colors:
if( !all(file.exists("./cached_results/b_daltonism__ncolors.RData")) )
{
  # Fit the model:
  b_daltonism__ncolors <- brm(n_colors ~ 1 + daltonism_r + I(daltonism_r^2)  + (1 | glottocode_family) + (1 | macroarea), # daltonism_r -> n_colors_r
                              family=gaussian(), data=d_colors, 
                              prior=c(prior(normal(0, 5), class="Intercept"),
                                      prior(normal(0, 5), class="b")), # pretty wide priors centered on 0
                              save_all_pars=TRUE, # needed for Bayes factors
                              sample_prior=TRUE,  # needed for hypotheses tests
                              cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_daltonism__ncolors <- brms_fit_indices(b_daltonism__ncolors);
  summary(b_daltonism__ncolors); mcmc_plot(b_daltonism__ncolors, type="trace"); mcmc_plot(b_daltonism__ncolors);
  (h_daltonism__ncolors <- brms::hypothesis(b_daltonism__ncolors, c("daltonism_r = 0", "Idaltonism_rE2 = 0")));
  (hdi_daltonism__ncolors <- hdi(b_daltonism__ncolors, ci=0.95));
  compare_0_daltonism__ncolors <- brms_compare_models(b_0__ncol, b_daltonism__ncolors, "null", "+ daltonism"); # possible effect of daltonism...
  
  # Try to remove the quadratic effect:
  b_daltonism__ncolors_1 <- update(b_daltonism__ncolors, . ~ . - I(daltonism_r^2), 
                                   save_all_pars=TRUE, sample_prior=TRUE, cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.999, max_treedepth=10));
  b_daltonism__ncolors_1 <- brms_fit_indices(b_daltonism__ncolors_1);
  summary(b_daltonism__ncolors_1); mcmc_plot(b_daltonism__ncolors_1, type="trace"); mcmc_plot(b_daltonism__ncolors_1);
  (h_daltonism__ncolors_1 <- brms::hypothesis(b_daltonism__ncolors_1, c("daltonism_r = 0")));
  (hdi_daltonism__ncolors_1 <- hdi(b_daltonism__ncolors_1, ci=0.95));
  compare_daltonism__ncolors_1 <- brms_compare_models(b_daltonism__ncolors, b_daltonism__ncolors_1, "full", "- qudratic"); # quadratic effect does not matter
  b_daltonism__ncolors <- b_daltonism__ncolors_1; h_daltonism__ncolors <- h_daltonism__ncolors_1; hdi_daltonism__ncolors <- hdi_daltonism__ncolors_1; # keep the linear only
  
  # Compare with null:
  compare_0_daltonism__ncolors <- brms_compare_models(b_0__ncol, b_daltonism__ncolors, "null", "+ n_colors"); # no real effect of daltonism...
  
  # Save results:
  save(compare_0_daltonism__ncolors, h_daltonism__ncolors, hdi_daltonism__ncolors, #b_daltonism__ncolors, # no need to save the actual model
       file="./cached_results/b_daltonism__ncolors.RData", compress="xz", compression_level=9);
} else
{
  load("./cached_results/b_daltonism__ncolors.RData");
}
```

Conversely, *daltonism* has no (or very weakly positive) effect on the number of colors categories (*n_colors*): *&beta;~daltonism~* = `r round(h_daltonism__ncolors$hypothesis$Estimate[1],2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_daltonism__ncolors$CI_low[ hdi_daltonism__ncolors$Parameter == "b_daltonism_r" ], hdi_daltonism__ncolors$CI_high[ hdi_daltonism__ncolors$Parameter == "b_daltonism_r" ])`), *p*(*&beta;*=0) = `r sprintf("%.2g",h_daltonism__ncolors$hypothesis$Post.Prob[1])`); and fits the data comparable to the null model (Bayes factor = `r sprintf("%.2g",compare_0_daltonism__ncolors$BF)`, LOO = `r sprintf("%.2f [SE=%.2f]", compare_0_daltonism__ncolors$LOO["null", "elpd_diff"], compare_0_daltonism__ncolors$LOO["null", "se_diff"])`, WAIC = `r sprintf("%.2f [SE=%.2f]", compare_0_daltonism__ncolors$WAIC["null", "elpd_diff"], compare_0_daltonism__ncolors$WAIC["null", "se_diff"])`, K-fold = `r sprintf("%.2f [SE=%.2f]", compare_0_daltonism__ncolors$KFOLD["null", "elpd_diff"], compare_0_daltonism__ncolors$KFOLD["null", "se_diff"])`).
...
Interestingly, the population frequency of daltonism is only weakly correlated with the inter-population genetic distances: a Bayesian mixed-effects quadratic Beta regression of daltonism with family and macorarea as random effects and the first 10 MDS dimensions of the ultrametrically-imputed genetic distances matrix with manual simplification is better than the null model (Bayes factor = `r sprintf("%.2g",compare_0_genetics__daltonism$BF)`, LOO = `r sprintf("%.2f [SE=%.2f]", compare_0_genetics__daltonism$LOO["null", "elpd_diff"], compare_0_genetics__daltonism$LOO["null", "se_diff"])`, WAIC = `r sprintf("%.2f [SE=%.2f]", compare_0_genetics__daltonism$WAIC["null", "elpd_diff"], compare_0_genetics__daltonism$WAIC["null", "se_diff"])`, K-fold = `r sprintf("%.2f [SE=%.2f]", compare_0_genetics__daltonism$KFOLD["null", "elpd_diff"], compare_0_genetics__daltonism$KFOLD["null", "se_diff"])`), has `r sprintf("R^2^= %.1f%% ±%.1f%%", r2_genetics__daltonism["R2", "Estimate"]*100, r2_genetics__daltonism["R2", "Est.Error"]*100)`, and the retained predictors are:
...
if( !all(file.exists(file_name)) )
{
  ...
  bmed_uvb_blue__daltonism <- .fit_mediation_model(d=d_colors, 
                                                   ...
                                                   treatment="UVB_r", treatment_name="UVB", 
                                                   mediator="exists_blue", mediator_name="blue", 
...
if( !all(file.exists(file_name)) )
{
  # The data:
  d_colors_lavaan <- d_colors[,c("glottocode", "glottocode_family", "macroarea", 
                                 "exists_blue", "n_colors_r", "UVB_r", "latitude_r", "dist2lakes_r", "clim_PC1_r", "subsistence", "log_popSize", "daltonism_r")];
  names(d_colors_lavaan) <- c("glottocode", "glottocode_family", "macroarea", 
                                 "blue", "ncol", "uvb", "lat", "d2l", "clim", "subs", "psiz", "dalt");
  # Binary variables recoding (as per https://lavaan.ugent.be/tutorial/cat.html):
  d_colors_lavaan$blue <- ordered(d_colors_lavaan$blue, levels=c("no","yes")); # this is an endogenous (dependent) variable -> make it ordered
  d_colors_lavaan$subs <- ordered(d_colors_lavaan$subs, levels=c("HG","AGR")); # this is an endogenous (dependent) variable -> make it ordered
  ...
  if( FALSE )
  {
    ...
    dalt ~~ ncol
 ';
    semfit_daltonism_full <- sem(sem_daltonism_full, data=d_colors_lavaan, se="robust.sem");
    summary(semfit_daltonism_full, fit.measures=TRUE, standardize=TRUE, rsquare=TRUE, estimates=TRUE, ci=TRUE);
    lavaanPlot(model=semfit_daltonism_full, coefs=TRUE, sig=1.00, stand=FALSE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
    semPaths(semfit_daltonism_full, what="est", edge.label.cex = 1.5, fade = FALSE)
    fitMeasures(semfit_daltonism_full, c("chisq", "df", "pvalue", "cfi", "tli", "nnfi", "rfi"));
    mi <- modindices(semfit_daltonism_full);
  }
 ...
 ';
  ...
  sem_daltonism <- list("data"=d_colors_lavaan,
                        "model"=semfit_daltonism_essential);
...
```{r fig.cap=capFig('The path model with non-standardized coefficients, showing all estimated path estimates. Single-headed arrows represent regressions, double-headed arrows represent covariance and variance (when refering to the same node), and the arrows emerging from triangles are the intercepts; blue edges have negative estimates, red ones positive estimates.'), fig.width=15, fig.height=10}
semPaths(sem_daltonism$model, what="est", edge.label.cex=0.75, fade=FALSE, nCharNodes=0, posCol="red", negCol="blue", layout="tree2");
```

```{r fig.cap=capFig('The path model with standardized coefficients; conventions as above.'), fig.width=15, fig.height=10}
semPaths(sem_daltonism$model, what="std", edge.label.cex=0.75, fade=FALSE, nCharNodes=0, posCol="red", negCol="blue", layout="tree2");
```

```{r fig.cap=capFig('The path model with non-standardized coefficients, showing all path estimates with significance. Single-headed arrows represent regressions, while double-headed arrows represent covariance. Please note that this is not a "standard" SEM/path analysis diagram (see below for such a representation).')}
lavaanPlot(model=sem_daltonism$model, coefs=TRUE, sig=1.00, stand=FALSE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
```

```{r fig.cap=capFig('The path model with standardized coefficients (see @grace_interpreting_2005 about pitfalls in interpreting such coefficients) showing only the significant (at the 0.05 level) path estimates with significance.')}
lavaanPlot(model=sem_daltonism$model, coefs=TRUE, sig=0.05, stand=TRUE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
```

This model fits the data well[^interpret_sem]: `r sprintf("*&chi;*^2^(%d) = %.1f, *p* = %.3g; CFI = %.3f, TLI = %.3f, NNFI = %.3f, RMSEA = %.3f 90%%CI [%.3f, %.3f]", fitMeasures(sem_daltonism$model, c("df")), fitMeasures(sem_daltonism$model, c("chisq")), fitMeasures(sem_daltonism$model, c("pvalue")), fitMeasures(sem_daltonism$model, c("cfi")), fitMeasures(sem_daltonism$model, c("tli")), fitMeasures(sem_daltonism$model, c("nnfi")), fitMeasures(sem_daltonism$model, c("rmsea")), fitMeasures(sem_daltonism$model, c("rmsea.ci.lower")), fitMeasures(sem_daltonism$model, c("rmsea.ci.upper")))`.

The full model output is in [Appendix III. Path analysis for "daltonism"]), but we can see that:

- as above, *latitude* affects *UV-B* (-) and *blue* (-);
- *blue* is affected by *UV-B* (-) and *latitude* (-);
- *n_colors* is affected by *blue* (+);
- *daltonism* is affected by *UV-B* (-) and *blue* (+), and is not related to *n_colors*.
...
if( !all(file.exists(file_name)) )
{
  ...
  brms_dalto <- brm(daltonism_r ~ 1 
                    ...
                    sample_prior=TRUE,  # needed for hypotheses tests
                    cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10));
  summary(brms_dalto); mcmc_plot(brms_dalto, type="trace"); mcmc_plot(v);
  ...
  brms_dalto_1 <- update(brms_dalto, . ~ . - log_popSize - dist2rivers_r - elevation_r - clim_PC3_r - gen_D10_r - gen_D7_r - gen_D6_r - gen_D4_r - gen_D1_r - 
                           clim_PC2_r - hum_median - dist2ocean_r - hum_IQR - gen_D3_r - gen_D9_r - dist2lakes_r - gen_D5_r - latitude_r - UVB_r - n_colors_r - 
...
```{r fig.cap=capFig(paste0("Various measures of success using Bayesian mixed effects logistic regressions splitting the dataset randomly into 80% training/20% testing subsets, ",brms_daltonism$training_testing$replications," times. Boxplots show the spread of the traning/tesing values, while the solid horizontal lines show the values when using the full dataset.")), fig.width=2*2, fig.height=5}
...
if( !all(file.exists(file_name)) )
{
  ...
  ctree_daltonism <- list("data"=d,
                          "all_data"=list("model"=ctree_train, "R2"=r2_all, "RMSE"=rmse_all), 
                          "training_testing"=list("replications"   =n_train,
                                                  "performance"=list("success"=do.call(rbind, lapply(results, function(x) x$performance$success)))));
...
plot(ctree_daltonism$all_data$model, gp=gpar(fontsize = 11));
```
...
```{r fig.cap=capFig(paste0("Various measures of success using Bayesian mixed effects logistic regressions splitting the dataset randomly into 80% training/20% testing subsets, ",ctree_daltonism$training_testing$replications," times. Boxplots show the spread of the traning/tesing values, while the solid horizontal lines show the values when using the full dataset.")), fig.width=2*2, fig.height=5}
...
grid.arrange(ggplot(rforest_daltonism$results$pred_importance, 
             ...
             ggplot(rforest_daltonism$results$pred_importance, 
               ...
               theme_bw() +
               geom_boxplot(fill="salmon", color="black") +
               labs(x="Predictors (sorted by decreasing importance)", y="Unconditional") +
               theme(axis.text.y = element_text(angle=30, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=10),
                     axis.title=element_text(size=16),
                     legend.position="none") +
...
```{r fig.cap=capFig(paste0("Various measures of success using SVMs splitting the dataset randomly into 80% training/20% testing subsets, ",svm_daltonism$training_testing$replications," times. Boxplots show the spread of the traning/tesing values, while the solid horizontal lines show the values when using the full dataset.")), fig.width=2*2, fig.height=5}
...
```{r fig.cap=capFig(paste0("Specificity-based predictor importance from SVMs using ",svm_daltonism$training_testing$replications," traning/testing sets, and including or not *n_colors*.")), fig.width=4*3, fig.height=7*1}
...
There is no evidence for a link between the number of color categories and the frequency of daltonism (except through its correlation with the existence of a dedicated word for "blue").
...
Thanks to the [Huma-Num](https://www.huma-num.fr/) for access to their computer cluster.
...
```{r}
summary(sem_blue$model);
fitMeasures(sem_blue$model);
```
...
```{r}
summary(sem_daltonism$model);
fitMeasures(sem_daltonism$model);
```
...
if( !file.exists("./cached_results/blue_var_importance.RData") )
{
  ...
  for (it in 1:n_iter_gini)
  {
    ...
    rf.db = randomForest(exists_blue ~  UVB_r + daltonism  + subsistence + latitude_r + longitude_r  + log_popSize + clim_PC1 + clim_PC2   + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = d_colors, importance=TRUE)
    ...
    obj <- cforest(exists_blue ~  UVB_r + daltonism  + subsistence +  latitude_r +longitude_r  + log_popSize + clim_PC1 + clim_PC2  + dist2water_r + dist2ocean_r + dist2lakes_r + dist2rivers_r + n_colors + latitude_family_r + longitude_family_r, data = d_colors)
