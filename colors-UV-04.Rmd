...
- update the summary mediation diagram
...
For the statistical analyses, *n_colors* was z-scored (thus, the relationship between raw and transformed values is linear).
...
```{r fig.cap=capFig('The links supported by the mediation and regression analyses. Plese note that we changed latitude back to its "natural" coding such that it is highest at the poles.'), fig.width=7, fig.height=4}
DiagrammeR::grViz(paste0('
  digraph full_blue {
  ...
  # the nodes:
  node [shape = box, style = "filled", fillcolor = "gray95"];
  blue [label = "blue",        fillcolor="blue", fontcolor="white"]; 
  lat  [label = "latitude",    fillcolor="palevioletred1"]; 
  UVB  [label = "UV-B",        fillcolor="palevioletred1"]; 
  d2l  [label = "dist2lakes",  fillcolor="steelblue1"]; 
  clim [label = "clim_PC1",    fillcolor="springgreen"]; 
  ncol [label = "n_colors",    fillcolor="lightgoldenrod1"]; 
  psiz [label = "pop_size",    fillcolor="burlywood"]; 
  subs [label = "subsistence", fillcolor="gray90", fontcolor="gray50"]; 

  # the edges:
  edge [style = "solid", color = "black"];
  lat  -> blue [color="gray80",     fontcolor="gray80",     style="dashed", label=""];
  lat  -> UVB  [color="violetred4", fontcolor="violetred4", penwidth=3,     label="-"];
  UVB  -> blue [color="violetred4", fontcolor="violetred4", penwidth=3,     label="-"];
  d2l  -> blue [color="slateblue4", fontcolor="slateblue4", penwidth=3,     label="-"];
  clim -> blue [color="darkgreen",  fontcolor="darkgreen",  penwidth=1,     label="(+)"];
  lat  -> clim [color="darkgreen",  fontcolor="darkgreen",  penwidth=3,     label="+"];
  ncol -> blue [color="yellow4",    fontcolor="yellow4",    penwidth=3,     label="+"];
  psiz -> ncol [color="burlywood4", fontcolor="violetred4", penwidth=3,     label="+"];
...
- *latitude* has no direct effect on "blue", but has a mediated positive effect (i.e., languages spoken at higher latitudes have a higher chance of having a dedicated word for "blue"); this is due to the composition of two negative effects: at higher latitudes, *UV-B incidence* is lower, which, in turn, increases the chance of a word for "blue";
- likewise, *latitude* influences *climate PC1*, which, in turn, affects "blue";
- *distance to lakes* has a negative effect on "blue";
- *subsistence strategy* does not have an effect on "blue";
- *population size* has a mediated positive effect on "blue", fully mediated by its positive effect on the *number of color categories*, which, in turn, has a positive effect on the existence of a dedicated work for "blue".
