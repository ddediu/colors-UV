...
author: "Mathilde Josserand [Mathilde.Josserand@univ-lyon2.fr], Emma Meeussen, Dan Dediu [ddediu@gmail.com] and Asifa Majid"
date: "`r date()`"
output:
  html_document:
    highlight: textmate
...
#### Including information on blue - paper format

```{r map word for blue 2 , fig.cap=capFig("Map of specific terms for blue.")}
# plot exists_blue on map
ggplot() + theme_bw(base_size = 18) + 
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=exists_blue, size=daltonism), alpha=0.7, color="red") +
  theme(legend.position = c(0.755, 0.60), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 11), 
      legend.text = element_text(size = 11)) +
  labs(fill = 'Term for "blue"?', size="Daltonism (%)") +
  scale_fill_viridis_d() + 
...
```{r map word for blue 2 bis , fig.cap=capFig("Map of specific terms for blue."), fig.width=11, fig.height=12}

library("gridExtra")


# plot exists_blue on map
pblue <- ggplot() + theme_bw(base_size = 18) + 
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=exists_blue), color="black", size=2.5, alpha=0.8) +
...
# plot exists_blue on map
pdalt <- ggplot() + theme_bw(base_size = 18) + 
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
  geom_point(data= d_colors, shape=21, aes(x = longitude, y = latitude, fill=daltonism), color="black", size=2.5, alpha=0.8) +
  theme(legend.position = c(0.79, 0.55), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 13), 
      legend.text = element_text(size = 13)) +
  labs(fill = 'Daltonism (%)') +
...
#### UVB &rarr; subsistence &rarr; "blue"
...
#### UVB &rarr; population size &rarr; "blue"
...
```{r}
load("./cached_results/sem_blue.RData");
```
...
#load("/media/mathilde/Elements/DDL_computer/Documents/Colors/colors-UV-master/cached_results/brms_blue.RData");
...
#### latitude &rarr; UVB &rarr; daltonism
...
#### Overall figure for predicting blue and daltonism


```{r computation predic blue, include=FALSE}

number_of_predictors <- 7

### --- BRMS
brms_res <- brms_blue$training_testing$both$predictors
# select best predictors according to previous finding (Bayes factor/WAIC/LOO/Kfold)
brms_res <- brms_res[brms_res$predictor == "UVB_r" | brms_res$predictor == "log_popSize" | brms_res$predictor == "dist2lakes_r" | brms_res$predictor == "gen_D7_r",]
# change name variables
brms_res%>%mutate(predictor=recode(predictor, log_popSize="popSize", UVB_r="UV-B", dist2lakes_r="dist2lak", gen_D7_r="genD7")) -> brms_res

### --- SVM
svm_res <- svm_blue$training_testing$full$pred_importance
svm_res_agg <- aggregate(x = svm_res$importance, by= list(svm_res$predictor), FUN=median)
svm_res_agg <- svm_res_agg[order(-svm_res_agg$x),] 
svm_res_final <- svm_res[svm_res$predictor %in% c(as.character(svm_res_agg[1:number_of_predictors,1])),]
# change name variable
svm_res_final%>%mutate(predictor=recode(predictor, log_popSize="popSize", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f")) -> svm_res_final


### --- RFOREST
rfor_res <- rforest_blue$full$pred_importance

## Accuracy
# find the number_of_predictors best variables
rfor_acc_agg <- aggregate(x = rfor_res$MeanDecreaseAccuracy, by= list(rfor_res$predictor), FUN=median)
rfor_acc_agg <- rfor_acc_agg[order(-rfor_acc_agg$x),] 
rfor_acc <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_acc_agg[1:number_of_predictors,1])),]
# change name variable
rfor_acc%>%mutate(predictor=recode(predictor, log_popSize="popSize",UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1")) -> rfor_acc

## Gini
rfor_gini_agg <- aggregate(x = rfor_res$MeanDecreaseGini, by= list(rfor_res$predictor), FUN=median)
rfor_gini_agg <- rfor_gini_agg[order(-rfor_gini_agg$x),] 
rfor_gini <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_gini_agg[1:number_of_predictors,1])),]
# change name variable
rfor_gini%>%mutate(predictor=recode(predictor, log_popSize="popSize", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f")) -> rfor_gini

## Conditional forest
rfor_cf_agg <- aggregate(x = rfor_res$cf_unconditional, by= list(rfor_res$predictor), FUN=median)
rfor_cf_agg <- rfor_cf_agg[order(-rfor_cf_agg$x),] 
rfor_cf <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_cf_agg[1:number_of_predictors,1])),]
# change name variable
rfor_cf%>%mutate(predictor=recode(predictor, log_popSize="popSize", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f")) -> rfor_cf

### --- Performance

# dataset with random forest and conditional random forest
mydataset_rf_cf <- rbind(cbind(rforest_blue$full$success, "full"="yes")) %>% 
         reshape2::melt(id.vars=c("replication", "full"), variable.name="measure", value.name="value") %>% mutate("method"=factor(if_else(substring(measure,1,2) == "rf", "RF", "CF"), levels=c("RF", "CF")), 
                "measure"=factor(substring(measure,4), levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), 
                "value"=value*100)
mydataset_rf_cf$full <- NULL

# dataset with brms
mydataset_bay <- brms_blue$training_testing$both$success %>% 
           reshape2::melt(id.vars=c("replication"), variable.name="measure", value.name="value") %>% 
           mutate("measure"=factor(measure, levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), "value"=value*100)
mydataset_bay$method <- "BRMS"

#  dataset with svm
mydataset_svm <- svm_blue$training_testing$full$success %>% 
         reshape2::melt(id.vars=c("replication"), variable.name="measure", value.name="value") %>% 
         mutate("measure"=factor(measure, levels=c("accuracy", "sensitivity", "specificity", "precision", "recall")), "value"=value*100)
mydataset_svm$method <- "SVM"

# merge datasets
all_data <- rbind(mydataset_rf_cf, mydataset_bay, mydataset_svm)
all_data$method <- factor(all_data$method, levels = c("CF", "RF", "SVM", "BRMS"))

# select only accuracy measure
all_data <- all_data[all_data$measure=="accuracy",]

```

```{r graph predic blue, include = FALSE}
predic_blue <- grid.arrange(ggplot(brms_res, 
                    aes(x=predictor, y=estimate)) +
               theme_bw() +
               geom_boxplot(fill="red", color="black") +
               labs(x="Best retained predictors", y="β") +
               theme(axis.text.y = element_text(angle=40, hjust=1, size=10), 
            ...
            ggplot(svm_res_final, 
                    aes(x=reorder(predictor, importance, median), y=importance)) +
               theme_bw() +
               geom_boxplot(fill="yellow", color="black") +
               labs(x="Predictors (by decreasing importance)", y="Importance") +
               theme(axis.text.y = element_text(angle=40, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=8),
                     axis.title=element_text(size=13),
                     legend.position="none") +
             ...
             ggplot(rfor_acc, 
                    aes(x=reorder(predictor, MeanDecreaseAccuracy, median), y=MeanDecreaseAccuracy)) +
               theme_bw() +
              #geom_boxplot(fill="#3CBB75FF", color="black") +
              geom_boxplot(fill="yellow", color="black") +
               labs(x="", y="Importance") +
               theme(axis.text.y = element_text(angle=40, hjust=1, size=10), 
                     axis.text.x = element_text(hjust=1, size=8),
             ...
             ggplot(rfor_cf, 
               ...
               coord_flip() + 
               NULL,
            ggplot(all_data, aes(x=method, y=value, fill=method))+
              theme_bw() +
              ylim(0,100) +
              scale_fill_manual(values = c("BRMS"="deepskyblue", "SVM"="deepskyblue",
                                           "RF"="deepskyblue","CF" =  "deepskyblue"))+
              geom_boxplot() +
              guides(fill=FALSE) +
              labs(x="Method", y="Accuracy (in %)") +
               theme(axis.text.y = element_text(angle=40, hjust=1, size=10),
                     axis.text.x = element_text(hjust=1, size=8),
                     axis.title=element_text(size=13),
                     legend.position="none") +
              ...
              coord_flip() +
              NULL,
             ncol=6, top = textGrob("A.",gp=gpar(fontsize=18,font=1)));
```

```{r computation predic dalto, include = FALSE}
number_of_predictors <- 7

### --- BRMS
brms_res <- brms_daltonism$training_testing$performance$predictors
# select best predictos
brms_res <- brms_res[brms_res$predictor == "longitude_r" | brms_res$predictor == "clim_PC1_r" | brms_res$predictor == "dist2water_r" | brms_res$predictor == "exists_blueyes" | brms_res$predictor == "gen_D8_r",]
# change names variables
brms_res%>%mutate(predictor=recode(predictor, longitude_r="long", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f", dist2water_r="dist2wat", exists_blueyes="blue", gen_D8_r="genD8")) -> brms_res


### --- SVM
svm_res <- data.frame(svm_daltonism$training_testing$performance$pred_importance)
svm_res_agg <- aggregate(x = svm_res$importance, by= list(svm_res$predictor), FUN=median)
svm_res_agg <- svm_res_agg[order(-svm_res_agg$x),] 
svm_res_final <- svm_res[svm_res$predictor %in% c(as.character(svm_res_agg[1:number_of_predictors,1])),]
# change names variables
svm_res_final%>%mutate(predictor=recode(predictor, longitude_r="long", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f", dist2water_r="dist2wat", exists_blueyes="blue", gen_D4_r="genD4", gen_D3_r="genD3", gen_D8_r="genD8")) -> svm_res_final

### --- RFOREST
rfor_res <- rforest_daltonism$results$pred_importance

## Accuracy
rfor_acc_agg <- aggregate(x = rfor_res$X.IncMSE, by= list(rfor_res$predictor), FUN=median)
rfor_acc_agg <- rfor_acc_agg[order(-rfor_acc_agg$x),] 
rfor_acc <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_acc_agg[1:number_of_predictors,1])),]
# change names variables
rfor_acc%>%mutate(predictor=recode(predictor, longitude_r="long", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f", dist2water_r="dist2wat", exists_blueyes="blue", gen_D4_r="genD4", gen_D3_r="genD3", gen_D8_r="genD8")) -> rfor_acc
...
## Node purity
rfor_np_agg <- aggregate(x = rfor_res$IncNodePurity, by= list(rfor_res$predictor), FUN=median)
rfor_np_agg <- rfor_np_agg[order(-rfor_np_agg$x),] 
rfor_np <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_np_agg[1:number_of_predictors,1])),]
# change names variables
rfor_np%>%mutate(predictor=recode(predictor, longitude_r="long", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f", dist2water_r="dist2wat", exists_blueyes="blue", gen_D4_r="genD4", gen_D3_r="genD3", gen_D8_r="genD8")) -> rfor_np
...
rfor_cf <- rfor_res[rfor_res$predictor %in% c(as.character(rfor_cf_agg[1:number_of_predictors,1])),]
# change names variables
rfor_cf%>%mutate(predictor=recode(predictor, longitude_r="long", UVB_r="UV-B", hum_median="humid_m", latitude_r="lat", dist2lakes_r="dist2lak", gen_D7_r="genD7", clim_PC1_r="climPC1", dist2ocean_family_r="dist2oc_f", clim_PC2_family_r="climPC2_f", clim_PC1_family_r="climPC1_f", macroarea="macroar", hum_median_family="humid_f", dist2water_r="dist2wat", exists_blueyes="blue", gen_D4_r="genD4", gen_D3_r="genD3", gen_D8_r="genD8")) -> rfor_cf


### --- Performance
success_rf <- data.frame(replication = rforest_daltonism$results$success$replication, R2 = rforest_daltonism$results$success$rf_R2, RMSE = rforest_daltonism$results$success$rf_RMSE)
success_rf$method = "RF"
success_cf <- data.frame(replication = rforest_daltonism$results$success$replication, R2 = rforest_daltonism$results$success$rf_R2, RMSE = rforest_daltonism$results$success$rf_RMSE)
success_cf$method = "CF"
success_bay <-  brms_daltonism$training_testing$performance$success
success_bay$method = "BRMS"
success_svm <- svm_daltonism$training_testing$performance$success
success_svm$method = "SVM"
acc_all <- rbind(success_rf, success_cf, success_bay, success_svm)
acc_all$method <- factor(acc_all$method, levels = c("CF", "RF", "SVM","BRMS"))

```


```{r graph predic dalto, include = FALSE}
predic_dalto <- grid.arrange(ggplot(brms_res, 
                    aes(x=predictor, y=estimate)) +
               theme_bw() +
               geom_boxplot(fill="red", color="black") +
               labs(x="Best retained predictors", y="β") +
            ...
            ggplot(svm_res_final, 
             ...
             ggplot(rfor_cf, 
               ...
               coord_flip() + 
               NULL,
            ggplot(acc_all, aes(x=method, y=R2*100, fill=method))+
              theme_bw() +
              ylim(0,100) +
              scale_fill_manual(values = c("BRMS"="deepskyblue","SVM"="deepskyblue", "RF"="deepskyblue","CF" =  "deepskyblue"))+
              geom_boxplot() +
              guides(fill=FALSE) +
              labs(x="Method", y=expression(R^{2})) +
               theme(axis.text.y = element_text(angle=40, hjust=1, size=10),
                     axis.text.x = element_text(hjust=1, size=8),
                     axis.title=element_text(size=13),
                     legend.position="none") +
              ...
              coord_flip() +
              NULL,
             ncol=6, top = textGrob("B.",gp=gpar(fontsize=18,font=1)));
```


```{r graph predic blue dalto, fig.cap=capFig(paste0("Summary of Beta estimates from BRMS, predictors importance from SVM, Random Forest and Conditional Random forest, and performance using ",svm_daltonism$training_testing$replications," training/testing sets.")), fig.width=15, fig.height=10}

grid.arrange(predic_blue, predic_dalto, nrow=2)
```
