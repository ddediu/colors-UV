...
if( !all(file.exists("./cached_results/b_0__blue.RData", "./cached_results/b_ncol__blue.RData")) )
{
  ...
  b_0__blue <- update(b_ncol__blue, . ~ . - n_colors, cores=brms_ncores, control=list(adapt_delta=0.99, max_treedepth=10)); # the null model for exists_blue
  b_0__blue <- brms_fit_indices(b_0__blue);
  summary(b_0__blue); mcmc_plot(b_0__blue, type="trace"); mcmc_plot(b_0__blue);
  (h_ncol__blue <- brms::hypothesis(b_ncol__blue, "n_colors = 0"));
...
ggplot(d_colors, aes(x=n_colors, fill=exists_blue)) + 
  ...
  scale_fill_manual('Has specific word for blue?', values=c("yes"="blue", "no"="red")) + 
  NULL;
```
...
Here we check if there is any relationship between measures of UV incidence and the existence of a word for "blue".
...
if( !all(file.exists("./cached_results/b_uv__blue.RData")) )
{
  # Individually:
  # UV_A_mean:
  b_uvam__blue <- brm(exists_blue ~ 1 + UV_A_mean_r + (1 | glottocode_family) + (1 | macroarea), # UV_A_mean -> exists_blue
                      family=bernoulli(link="logit"), data=d_colors, 
                      prior=c(prior(student_t(3, 0, 2.5), class="Intercept"),
                              prior(student_t(3, 0, 2.5), class="b")), # pretty wide priors centered on 0
                      save_all_pars=TRUE, # needed for Bayes factors
  ...
  (h_uvam__blue <- brms::hypothesis(b_uvam__blue, "UV_A_mean_r = 0"));
  (hdi_uvam__blue <- hdi(b_uvam__blue, ci=0.95));
  compare_0_uvam__blue <- brms_compare_models(b_0__blue, b_uvam__blue, "null", "+ UV_A_mean"); # clear negative effect of UV_A_mean!
  
  # UV_A_sd:
  b_uvas__blue <- brm(exists_blue ~ 1 + UV_A_sd_r + (1 | glottocode_family) + (1 | macroarea), # UV_A_sd -> exists_blue
                      family=bernoulli(link="logit"), data=d_colors, 
                      prior=c(prior(student_t(3, 0, 2.5), class="Intercept"),
                              prior(student_t(3, 0, 2.5), class="b")), # pretty wide priors centered on 0
                      save_all_pars=TRUE, # needed for Bayes factors
  ...
  (h_uvas__blue <- brms::hypothesis(b_uvas__blue, "UV_A_sd_r = 0"));
  ...
  # UV_B_mean:
  b_uvbm__blue <- brm(exists_blue ~ 1 + UV_B_mean_r + (1 | glottocode_family) + (1 | macroarea), # UV_B_mean -> exists_blue
                      family=bernoulli(link="logit"), data=d_colors, 
                      prior=c(prior(student_t(3, 0, 2.5), class="Intercept"),
                              prior(student_t(3, 0, 2.5), class="b")), # pretty wide priors centered on 0
                      save_all_pars=TRUE, # needed for Bayes factors
  ...
  (h_uvbm__blue <- brms::hypothesis(b_uvbm__blue, "UV_B_mean_r = 0"));
  (hdi_uvbm__blue <- hdi(b_uvbm__blue, ci=0.95));
  compare_0_uvbm__blue <- brms_compare_models(b_0__blue, b_uvbm__blue, "null", "+ UV_B_mean"); # clear negative effect of UV_B_mean!
  
  # UV_B_sd:
  b_uvbs__blue <- brm(exists_blue ~ 1 + UV_B_sd_r + (1 | glottocode_family) + (1 | macroarea), # UV_B_sd -> exists_blue
                      family=bernoulli(link="logit"), data=d_colors, 
                      prior=c(prior(student_t(3, 0, 2.5), class="Intercept"),
                              prior(student_t(3, 0, 2.5), class="b")), # pretty wide priors centered on 0
                      save_all_pars=TRUE, # needed for Bayes factors
  ...
  (h_uvbs__blue <- brms::hypothesis(b_uvbs__blue, "UV_B_sd_r = 0"));
...
Fitting individual Bayesian mixed-effects logistic regressions (with family and macroarea as random effects) predicting *exists_blue* from the mean and standard deviation of the incidence of UV-A and UV-B for 1998, we found that:
...
- the means (*UV_A_mean* and *UV_B_mean*) both have predictive negative effects on *exists_blue* (*&beta;~UV_A_mean~* = `r round(h_uvam__blue$hypothesis$Estimate,2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_uvam__blue$CI_low[ hdi_uvam__blue$Parameter == "b_UV_A_mean_r" ], hdi_uvam__blue$CI_high[ hdi_uvam__blue$Parameter == "b_UV_A_mean_r" ])`), *p*(*&beta;*=0) = `r sprintf("%.3g",h_uvam__blue$hypothesis$Post.Prob[1])`; *&beta;~UV_B_mean~* = `r round(h_uvbm__blue$hypothesis$Estimate,2)` (95%HDI = `r sprintf("[%.2f, %.2f]", hdi_uvbm__blue$CI_low[ hdi_uvbm__blue$Parameter == "b_UV_B_mean_r" ], hdi_uvbm__blue$CI_high[ hdi_uvbm__blue$Parameter == "b_UV_B_mean_r" ])`), *p*(*&beta;*=0) = `r sprintf("%.3g",h_uvbm__blue$hypothesis$Post.Prob[1])`),
