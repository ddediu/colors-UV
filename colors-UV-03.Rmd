...
grid.arrange(ggplot(d_colors, aes(y=latitude, x=exists_blue, fill=exists_blue)) + 
               ...
               scale_fill_manual('Word for blue?', values=c("yes"="blue", "no"="green")) + scale_color_manual('Word for blue?', values=c("yes"="blue", "no"="green")) + 
               NULL, 
             ggplot(d_colors, aes(y=abs(latitude), x=exists_blue, fill=exists_blue)) + 
               geom_jitter(alpha=0.50, aes(color=exists_blue)) + geom_violin(alpha=0.25, aes(color=exists_blue)) + geom_boxplot(fill=alpha("gray80",0.30)) + 
               theme_bw() +
               labs(x='Word for blue?', y="abs(latitude) (°)") +
               theme(axis.text.x = element_text(size =14, face = "bold"), 
                     axis.text.y = element_text(hjust = 1, size =14, face = "bold"),
                     axis.title=element_text(size=14),
                     legend.title=element_text(size =16),
...
grid.arrange(ggplot(d_colors, aes(y=latitude_family, x=exists_blue, fill=exists_blue)) + 
               ...
               scale_fill_manual('Word for blue?', values=c("yes"="blue", "no"="green")) + scale_color_manual('Word for blue?', values=c("yes"="blue", "no"="green")) + 
               NULL, 
             ggplot(d_colors, aes(y=abs(latitude_family), x=exists_blue, fill=exists_blue)) + 
               geom_jitter(alpha=0.50, aes(color=exists_blue)) + geom_violin(alpha=0.25, aes(color=exists_blue)) + geom_boxplot(fill=alpha("gray80",0.30)) + 
               theme_bw() +
               labs(x='Word for blue?', y="abs(latitude) (°)") +
               theme(axis.text.x = element_text(size =14, face = "bold"), 
...
if( !all(file.exists("./cached_results/b_lat__blue.RData")) )
{
  ...
  # For families:
  bf_lat__blue_all <- brm(exists_blue ~ 1 + latitude_family_r + I(latitude_family_r^2) + abs(latitude_family_r) + (1 | glottocode_family) + (1 | macroarea), # latitude -> exists_blue
                          family=bernoulli(link="logit"), data=d_colors, 
                          prior=c(prior(student_t(3, 0, 2.5), class="Intercept"),
                                  prior(student_t(3, 0, 2.5), class="b")), # pretty wide priors centered on 0
                          save_all_pars=TRUE, # needed for Bayes factors
  ...
  (hf_lat__blue <- brms::hypothesis(bf_lat__blue_all, c("latitude_family_r = 0", "latitude_family_r < 0", "Ilatitude_family_rE2 = 0", "Ilatitude_family_rE2 < 0", "abslatitude_family_r = 0", "abslatitude_family_r < 0")));
  ...
  # Iterative manual simplification:
  bf_lat__blue_1 <- update(bf_lat__blue_all, . ~ . - abs(latitude_family_r) - I(latitude_family_r^2), 
                           cores=brms_ncores, iter=4000, warmup=2000, thin=2, control=list(adapt_delta=0.99, max_treedepth=10));
  bf_lat__blue_1 <- brms_fit_indices(bf_lat__blue_1);
  summary(bf_lat__blue_1); mcmc_plot(bf_lat__blue_1, type="trace"); mcmc_plot(bf_lat__blue_1);
  (hf_lat__blue_1 <- brms::hypothesis(bf_lat__blue_1, c("latitude_family_r = 0", "latitude_family_r < 0")));
  (hdif_lat__blue_1 <- hdi(bf_lat__blue_1, ci=0.95));
  brms_compare_models(bf_lat__blue_1, bf_lat__blue_all, "simplified", "full"); # simplified is marginally better
  compare_0_lat__blue <- brms_compare_models(b_0__blue, bf_lat__blue_1, "null", "+ latitude"); # no effect of latitude...
