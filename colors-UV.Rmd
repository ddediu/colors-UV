...
This `HTML` document (possibly, printed to `PDF`) resulted from compiling the corresponding `Rmarkdown` script and contains all the results and plots supporting the paper *Environment and culture shape both the colour lexicon and the genetics of colour perception*.
All the data and scripts needed to reproduce this document are available in the *GitHub* repository [https://github.com/ddediu/colors-UV](https://github.com/ddediu/colors-UV) and on *Zenodo* at [doi:10.5281/zenodo.5083676](doi://10.5281/zenodo.5083676).
...
- *raw output*, as produced by various `R` functions and expressions (for example, as in the [Appendices](#appendix-ii.-path-analysis-for-blue)), is shown using `fixed font text` in clearly marked boxes;
...
```{r distribution UV-A, fig.cap=capFig("UV-A (mean and standard deviation).", TRUE, c("ggplot2"), is_map=FALSE)}
...
```{r map UV-A, fig.cap=capFig("Map of UV-A (mean and standard deviation).", TRUE, c("ggplot2", "maps"), is_map=TRUE), fig.height=12}
...
```{r distribution UV-B, fig.cap=capFig("UV-B (mean and standard deviation).", TRUE, c("ggplot2"), is_map=FALSE)}
...
```{r map UV-B, fig.cap=capFig("Map of UV-B (mean and standard deviation).", TRUE, c("ggplot2", "maps"), is_map=TRUE), fig.height=12}
grid.arrange(ggplot() + theme_bw() +
               geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
...
```{r distribution UV, fig.cap=capFig("UV (mean and standard deviation).", TRUE, c("ggplot2"), is_map=FALSE)}
...
```{r map UV, fig.cap=capFig("Map of UV (mean and standard deviation).", TRUE, c("ggplot2", "maps"), is_map=TRUE), fig.height=12}
grid.arrange(ggplot() + theme_bw() +
               geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
...
```{r distribution popsize, fig.cap=capFig("Log population size.", TRUE, c("ggplot2"), is_map=FALSE)}
...
```{r map popsize, fig.cap=capFig("Map of log population size.", TRUE, c("ggplot2", "maps"), is_map=TRUE)}
ggplot() + theme_bw() +
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
...
```{r distribution of daltonism, fig.cap=capFig("Incidence of red/green abnormal color perception.", TRUE, c("ggplot2"), is_map=FALSE)}
# Plot
...
```{r map word for daltonism, fig.cap=capFig("Map of incidence of red/green abnormal color perception (color scale).", TRUE, c("ggplot2", "maps"), is_map=TRUE)}
ggplot() + theme_bw() + 
  geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
...
```{r plot Delaunay, fig.cap=capFig("Restricted Delaunay triangulation for the languages in our dataset (in a Pacific-centered view). The yellow dots are the language locations, the thin black lines the connections to their immediate neighbours, and the thick red lines represent barriers to movement.", TRUE, c("ggplot2", "maps"), is_map=TRUE)}
...
```{r G function, fig.cap=capFig("The *G*, *F* and *K* functions (one per panel, from left to right). *r* is the distance between points. Expected distribution: blue; all others are the observed distributions with various edge correction methods (see help for `Gest` in package `spatstat`).", TRUE, c("spatstat"), is_map=FALSE), fig.width=9, fig.height=4}
par(mfrow=c(1,3));
...
```{r fig.cap=capFig("Probability of having a specific word for 'blue' function of (log) population size, showing the jittered data points, the densities (colored violins) and boxplots (black).", TRUE, c("ggplot2"), is_map=FALSE), fig.height=1*5, fig.width=1*4}
...
```{r map MDS genetics, fig.cap=capFig("Map of the MDS dimensions of the genetic distances matrix that predict the existence of a specific word for 'blue' (color scale).", TRUE, c("ggplot2"), is_map=FALSE), fig.width=2*8, fig.height=2*5}
grid.arrange(ggplot() + theme_bw() +
               geom_polygon(data=mapWorld, aes(x=long, y=lat, group=group), fill="grey") + xlab("Longitude (°)") + ylab("Latitude (°)") + # landmasses
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the outcome is binary, the direct and indirect effects may be on different scales.', TRUE, c("DiagrammeR"), is_map=FALSE), fig.width=10, fig.height=5}
...
```{r fig.cap=capFig('The path model with standardized coefficients (see @grace_interpreting_2005 about pitfalls in interpreting such coefficients) showing only the significant (at the 0.05 level) path estimates with significance.', TRUE, c("lavaanPlot"), is_map=FALSE)}
...
```{r fig.cap=capFig('Conditional inference trees for predicting *blue* using all the potential predictors. Please note that some predictors are transformed (the "_r" suffix), the most important here being *macroarea*.', TRUE, c("partykit"), is_map=FALSE), fig.width=12, fig.height=7, fig.show='hold', out.height="50%"}
...
```{r fig.cap=capFig("Graphical representation of Hypothesis 2.", TRUE, c("DiagrammeR"), is_map=FALSE)}
DiagrammeR::grViz(paste0('
...
```{r fig.cap=capFig("Graphical representation of Hypothesis 2 with our measures as a SEM diagram.", TRUE, c("DiagrammeR"), is_map=FALSE)}
DiagrammeR::grViz(paste0('
...
```{r fig.cap=capFig("Graphical representation of Hypothesis 2 with our measures as a path analysis diagram.", TRUE, c("DiagrammeR"), is_map=FALSE)}
DiagrammeR::grViz(paste0('
...
```{r fig.cap=capFig("Population frequency of daltonism (in %) function of having a specific word for 'blue', showing the jittered data points, the densities (colored violins) and boxplots (black).", TRUE, c("ggplot2"), is_map=FALSE), fig.height=1*5, fig.width=1*4}
...
```{r fig.cap=capFig('Population frequency of daltonism (in %) function of UV-B incidence, showing the data points and the linear and LOESS trends (with 95% CIs).', TRUE, c("ggplot2"), is_map=FALSE), fig.height=1*5, fig.width=1*5}
...
```{r fig.cap=capFig('Population frequency of daltonism (in %) function of genetic distance between populations.', TRUE, c("ggplot2"), is_map=FALSE), fig.height=3*4, fig.width=3*5}
...
```{r fig.cap=capFig('Mediation analysis showing the total, direct and indirect effects, as well as the actual regression coefficients. Please note that because the mediator is binary, the direct and indirect effects may be on different scales.', TRUE, c("DiagrammeR"), is_map=FALSE), fig.width=10, fig.height=5}
...
```{r fig.cap=capFig('Conditional inference trees for predicting *daltonism* using all the potential predictors.', TRUE, c("partykit"), is_map=FALSE), fig.width=10, fig.height=6, fig.show='hold', out.height="50%"}
...
```{r fig.cap=capFig(paste0("Various measures of success using random forests (RF) and conditional random forests (CRF)."), TRUE, c("ggplot2"), is_map=FALSE), fig.width=4*2, fig.height=1*5}
...
```{r fig.cap=capFig(paste0("Predictor importance from random forests using ",rforest_blue$replications," replications using random forests (left: accuracy-based, middle: Gini-index based) and conditional random forests (right)."), TRUE, c("ggplot2"), is_map=FALSE), fig.width=4*3, fig.height=7*1}
...
```{r fig.cap=capFig(paste0("Specificity-based predictor importance from SVMs using ",svm_daltonism$training_testing$replications," training/testing sets."), TRUE, c("ggplot2"), is_map=FALSE), fig.width=4*1, fig.height=7*1}
