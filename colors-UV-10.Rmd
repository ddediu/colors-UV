...
## Various folders:
setwd("/media/mathilde/Elements/DDL_computer/Documents/Colors/Color_final")
#setwd("F:/DDL_computer/Documents/Colors/Color_nouveau")
...
```{r populations map 2, fig.cap=capFig("Map of the populations in our sample (with language names)."), , fig.height=10, fig.width=11}
...
- *latitude* has no direct effect on "blue", but is mediated mostly by the negative one of *UV-B incidence* and, possibly, by the negative one of *subsistence* and *dist2lakes*;
- similarly, *climate PC1* has no direct effect on blue, but its effect is mediated by the negative effect on *subsistence*;
- *subsistence strategy* does affect "blue" only through its mediating effect with *population size*;
...
- *latitude* seems to have a direct influence on all the other measures: not only on *UV-B* (-), but also on *climate PC1* (+), *distance to lakes* (-), and even *population size* (-), the strongest influences being on *subsistence* and *UV-B*. 
- *blue* is affected by *UV-B* (-), *distance to lakes* (-) and *population size* (+), the first strongly supporting the hypothesis of a physiological effect of UV-B incidence, and the latter supporting the idea that "cultural complexity" promotes more complex color vocabularies
- *climate* influences only *population size* (+);
- *subsistence* influences *blue* only through its mediating effect with *population size* (+).

We did include *longitude* in our model (in order to better model the geographical patterning of linguistic diversity), but it did not seem to add anything.
  ...
  results <- pblapply(1:n_train, function(i) # very computationally expensive
    {
      ...
      # confusion matrices:
      cm_brms_full   <- confusionMatrix(data_test$exists_blue, factor(ifelse(predict(brms_blue__full,   newdata=data_test, allow_new_levels=TRUE)[,"Estimate"] < 0.5, 
                                                                            "no", "yes"), levels=c("no", "yes")), positive="yes");
      # predictors:
      hdi_full   <- hdi(brms_blue__full, ci=0.95); rope_full   <- rope(brms_blue__full,   ci=0.95, verbose=FALSE);
      ...
      preds_full <- data.frame("replication"=i, # replication,
                               "predictor"  =rownames(fixef(brms_blue__full)), 
                               "estimate"   =fixef(brms_blue__full)[,"Estimate"],
...
- consistently, the most relevant predictors are: *population size*, *UV-B incidence*, *climate*/*humidity*, *latitude* and *distance to large bodies of standing water* (mostly lakes but also sometimes seas/oceans).
...
Thus, the presence of a dedicated word for "blue" is partly explained by the population size and a few environmental variables concerning UV-B radiation, climate and distance to large bodies of standing water.
...
```{r fig.cap=capFig('Graphical representation of the full Hypothesis 2 with our measures as a SEM diagram. Edges: solid blue = negative effects, solid red = positive effects, and dashed gray = null effects; the dashed red edge is borderline significant. Nodes: "blue" in blue is the outcome, "UV-B" in green is of particular interest here, light yellow have an effect on "blue", while light gray do not. This should *not* be interpreted as a path diagram!'), fig.width=7, fig.height=4}
...
if( !all(file.exists(file_name)) )
{
  ...
  summary(semfit_daltonism_full, fit.measures=TRUE, standardize=TRUE, rsquare=TRUE, estimates=TRUE, ci=TRUE);
    lavaanPlot(model=semfit_daltonism_full, coefs=TRUE, sig=1, stand=FALSE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));

  lavaanPlot(model=semfit_daltonism_full, coefs=TRUE, sig=0.05, stand=TRUE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
 ...
 ';
  ...
  sem_daltonism <- list("data"=d_colors_lavaan,
                        ...
                        "model_full"=semfit_daltonism_full);
...
Finally, we observe the full model including all variables, as shown in the path analysis created for blue, and where we added *daltonism*: 

```{r fig.cap=capFig('The path model with non-standardized coefficients, showing all path estimates with significance. Single-headed arrows represent regressions, while double-headed arrows represent covariance. Please note that this is not a "standard" SEM/path analysis diagram.')}
lavaanPlot(model=sem_daltonism$model_full, coefs=TRUE, sig=1, stand=FALSE, covs=TRUE, stars=c("regress", "latent", "covs"), edge_options=list(color="gray"));
# Please note that we could not print the standardized coefficient here (error in https://github.com/alishinski/lavaanPlot/issues/23)
```

This model also fits the data well[^interpret_sem]: `r sprintf("*&chi;*^2^(%d) = %.1f, *p* = %.3g; CFI = %.3f, TLI = %.3f, NNFI = %.3f, RMSEA = %.3f 90%%CI [%.3f, %.3f]", fitMeasures(sem_daltonism$model_full, c("df")), fitMeasures(sem_daltonism$model_full, c("chisq")), fitMeasures(sem_daltonism$model_full, c("pvalue")), fitMeasures(sem_daltonism$model_full, c("cfi")), fitMeasures(sem_daltonism$model_full, c("tli")), fitMeasures(sem_daltonism$model_full, c("nnfi")), fitMeasures(sem_daltonism$model_full, c("rmsea")), fitMeasures(sem_daltonism$model_full, c("rmsea.ci.lower")), fitMeasures(sem_daltonism$model_full, c("rmsea.ci.upper")))`.
...
- *daltonism* is affected only *blue* (+); however, when considering the full set of predictors, *daltonism* is affected only by *UV-B* and *subsistence*, and *blue* is not part of its significant predictors.
- *latitude* has a direct effect on *daltonism* that does not go through any of our variables
...
The full output of this model with all variables, with all the fit indices, is:

```{r}
summary(sem_daltonism$model_full);
fitMeasures(sem_daltonism$model_full);
```
